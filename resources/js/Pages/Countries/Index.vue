...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('system_management.countries.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
// ─── Columns ─────────────────────────────────────────────────────────────
const allColumns = computed(() => countriesTableColumns(t, { isMobile: isMobileScreen.value }));
...
const VIEW_KEY = 'countries_view_mode';
...
const goToEditAll = () => router.visit(route('system_management.countries.edit_all'));
...
// ─── Onboarding tour ─────────────────────────────────────────────────────
const tour = useModuleTour({ module: 'countries', steps: () => moduleTourSteps(t, { moduleName: t('countries.plural') }) });
</script>
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="countries" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('countries.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('countries.create') && canUsePlanFeature('imports')" key="import" @click="openImport">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('countries.edit') && canUsePlanFeature('edit_all')" key="editall" @click="goToEditAll">
                ...
                <Tooltip v-if="can('countries.create')" :title="$t('countries.new')" data-tour="new">
                    <Link :href="route('system_management.countries.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('countries.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
            ...
            >
                ...
                <template #bodyCell="{ column, record, isMobile, compact, text }">
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('system_management.countries.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
