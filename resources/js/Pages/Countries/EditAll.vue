...
import { SaveOutlined, UndoOutlined, EditOutlined, FlagOutlined } from '@ant-design/icons-vue';
...
<template>
    ...
    <div class="edit-all sap-form">
        ...
        <div v-if="countries.total > countries.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('countries.edit_all_discard')"
            :save-label="$t('countries.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('countries.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
