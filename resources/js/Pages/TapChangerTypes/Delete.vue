...
const submit = () => {
    form.delete(route('business_management.tap_changer_types.deleteSave', props.tapChangerType.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('tap_changer_types.singular')" />

    <DeletePage
        :back-href="route('business_management.tap_changer_types.index')"
        :title="$t('global.delete') + ' ' + $t('tap_changer_types.record')"
        :subtitle="tapChangerType.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('tap_changer_types.name')">{{ tapChangerType.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="tapChangerType.code" :label="$t('tap_changer_types.code')">
                <code>{{ tapChangerType.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('tap_changer_types.is_active')">
                <Tag :color="tapChangerType.is_active ? 'success' : 'error'" :bordered="false">
