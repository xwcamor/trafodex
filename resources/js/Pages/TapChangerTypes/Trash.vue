...
import TapChangerTypesTrashBulkBar from '@/Components/TapChangerTypes/TapChangerTypesTrashBulkBar.vue';
import TapChangerTypesForceDeleteModal from '@/Components/TapChangerTypes/TapChangerTypesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { tapChangerTypesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.tap_changer_types.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.tap_changer_types.restore',
    bulkRestoreRouteName: 'business_management.tap_changer_types.bulk_restore',
});
...
const openForceDelete = (tapChangerType) => {
    forceDeleteTarget.value = tapChangerType;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.tap_changer_types.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => tapChangerTypesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.tapChangerTypes.total === 1 ? t('global.record') : t('global.records');
    return `${props.tapChangerTypes.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_types.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.tap_changer_types.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_types.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('tap_changer_types.restore_hint')"
            ...
            <Empty
                v-if="tapChangerTypes.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <TapChangerTypesTrashBulkBar
        ...
        <TapChangerTypesForceDeleteModal
