...
import TapChangerTypesBulkBar from '@/Components/TapChangerTypes/TapChangerTypesBulkBar.vue';
import TapChangerTypesBulkDeleteModal from '@/Components/TapChangerTypes/TapChangerTypesBulkDeleteModal.vue';
import TapChangerTypesFavoriteCell from '@/Components/TapChangerTypes/TapChangerTypesFavoriteCell.vue';
import TapChangerTypesPageHeader from '@/Components/TapChangerTypes/TapChangerTypesPageHeader.vue';
import TapChangerTypesActionsCell from '@/Components/TapChangerTypes/TapChangerTypesActionsCell.vue';
import TapChangerTypesEmptyState from '@/Components/TapChangerTypes/TapChangerTypesEmptyState.vue';
...
// El toolbar inline de TapChangerTypes (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    tapChangerTypesFilterFields, tapChangerTypesEmptyFilters, hydrateTapChangerTypesFilters,
    tapChangerTypesFiltersToQuery, tapChangerTypesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { tapChangerTypesTableColumns } from './config/columns';
import { tapChangerTypesExportableColumns, tapChangerTypesExportEndpoints } from './config/exports';
...
const props = defineProps({
    tapChangerTypes:      { type: Object, required: true },
...
const {
    ...
    hydrate:       hydrateTapChangerTypesFilters,
    toQuery:       tapChangerTypesFiltersToQuery,
    summary:       tapChangerTypesFiltersSummary,
    empty:         tapChangerTypesEmptyFilters,
    only:          ['tap_changer_types', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.tapChangerTypes),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    tapChangerTypesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.tapChangerTypes.current_page,
    pageSize: props.tapChangerTypes.per_page,
    total:    props.tapChangerTypes.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.tap_changer_types.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'tap_changer_types_view_mode';
...
useModuleUndoToast('business_management.tap_changer_types.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('tap_changer_types', 'tap_changer_types');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.tap_changer_types.bulk_set_active',
    bulkDeleteRoute:    'business_management.tap_changer_types.bulk_delete',
    resourceLabel:      t('tap_changer_types.records'),
});
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.tap_changer_types.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => tapChangerTypesExportableColumns(t));
const exportEndpoints   = computed(() => tapChangerTypesExportEndpoints());
...
const tour = useModuleTour({ module: 'tap_changer_types', steps: () => moduleTourSteps(t, { moduleName: t('tap_changer_types.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('tap_changer_types.create') && router.visit(route('business_management.tap_changer_types.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.tap_changer_types.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.tap_changer_types.delete', record.slug));
</script>

<template>
    <Head :title="$t('tap_changer_types.plural')" />

    <div class="sap-index">
        <!-- Título (izq) + acciones (Vistas, Exportar, engranaje, Nuevo) a la derecha. -->
        <div class="mi-title" data-tour="module">
            <TapChangerTypesPageHeader
                :title="$t('tap_changer_types.plural')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="tap_changer_types" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('tap_changer_types.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('tap_changer_types.create') && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('tap_changer_types.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.tap_changer_types.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.tap_changer_types.trash'))">
                ...
                <Tooltip v-if="can('tap_changer_types.create')" :title="$t('tap_changer_types.new')" data-tour="new">
                    <Link :href="route('business_management.tap_changer_types.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('tap_changer_types.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="tapChangerTypes.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('tap_changer_types.delete') || can('tap_changer_types.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <TapChangerTypesEmptyState
                        :has-filters="hasActiveFilters"
                        :can-create="can('tap_changer_types.create')"
                        @clear-filters="clearFilters"
                        @open-import="importOpen = true"
                    />
                </template>
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <TapChangerTypesFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === tapChangerTypes.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.tap_changer_types.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
                    ...
                    <TapChangerTypesActionsCell
                        v-else-if="column.key === 'actions'"
                        :record="record"
                        :is-mobile="isMobile"
                        :compact="compact"
                        :can-edit="can('tap_changer_types.edit')"
                        :can-create="can('tap_changer_types.create')"
                        :can-delete="can('tap_changer_types.delete')"
        ...
        <TapChangerTypesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('tap_changer_types.record') : $t('tap_changer_types.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('tap_changer_types.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="tapChangerTypes.total ?? 0"
            :total-unfiltered="tapChangerTypes.total_unfiltered ?? tapChangerTypes.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.tap_changer_types.import')"
            :template-url="route('business_management.tap_changer_types.import_template')"
            :resource-label="$t('tap_changer_types.records')"
        />
    </div>
</template>
