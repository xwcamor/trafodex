...
import TapChangerTypesEditAllTable from '@/Components/TapChangerTypes/TapChangerTypesEditAllTable.vue';
...
/**
 * Edit-All de TapChangerTypes. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.tapChangerTypes.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.tap_changer_types.edit_all.update'),
...
<template>
    <Head :title="$t('tap_changer_types.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.tap_changer_types.index')"
            :title="$t('global.edit_all') + ' — ' + $t('tap_changer_types.plural')"
            :subtitle="$t('tap_changer_types.edit_all_subtitle')"
        >
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('tap_changer_types.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <TapChangerTypesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="tapChangerTypes.total > tapChangerTypes.per_page" class="edit-pagination">
            <Pagination
                :current="tapChangerTypes.current_page"
                :pageSize="tapChangerTypes.per_page"
                :total="tapChangerTypes.total"
        ...
        <EditAllFooter
            :discard-label="$t('tap_changer_types.edit_all_discard')"
            :save-label="$t('tap_changer_types.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('tap_changer_types.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
