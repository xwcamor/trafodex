...
// ─── Filtros avanzados (builder inline "Agregar filtro" + drawer mobile) ────
// Array de clausulas {field, op, value}. Se persiste via Inertia
// (filters.advanced_where) para que sobreviva al paginate/sort.
...
const applyAdvancedFilters = (clauses) => {
    ...
    router.get(route('system_management.regions.index'), data, {
        preserveScroll: true,
        preserveState: true,
...
// Builder inline (desktop): aplica SOLO las cláusulas completas (debounce).
...
// ─── Columns ─────────────────────────────────────────────────────────────
const allColumns = computed(() => regionsTableColumns(t, { isMobile: isMobileScreen.value }));
...
const VIEW_KEY = 'regions_view_mode';
...
// ─── Onboarding tour ─────────────────────────────────────────────────────
const tour = useModuleTour({ module: 'regions', steps: () => moduleTourSteps(t, { moduleName: t('regions.plural') }) });
</script>
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="regions" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('regions.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('regions.create') && canUsePlanFeature('imports')" key="import" @click="openImport">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('regions.edit') && canUsePlanFeature('edit_all')" key="editall" @click="goToEditAll">
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('system_management.regions.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
                            </div>
                        </div>
                    </template>
