...
/**
 * Schema de filtros del módulo Regions. Toma `t` y devuelve los fields
 ...
 * por default.
 */
export const regionsFilterFields = (t) => [
    { key: 'name',       label: t('regions.name'),       type: 'tags' },
    { key: 'is_active',  label: t('regions.is_active'),  type: 'select', options: [
...
export const regionsEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
    updated_at: null,
...
export const hydrateRegionsFilters = (sf) => ({
...
export const regionsFiltersToQuery = (f, sf) => ({
...
export const regionsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('regions.name')}: ${f.name.join(', ')}`);
    if (f.is_active === true)  parts.push(`${t('regions.is_active')}: ${t('global.active')}`);
    if (f.is_active === false) parts.push(`${t('regions.is_active')}: ${t('global.inactive')}`);
