...
import { Card, Tag, Tooltip, Switch, Button, Badge, Dropdown, Menu, MenuItem, Input, Drawer } from 'ant-design-vue';
import {
...
const applySavedViewState = (clauses, meta) => {
    ...
    router.get(route('automation_management.automations.index'), data, {
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('automation_management.automations.index'), data, { preserveScroll: true, preserveState: true, onStart: () => { isFetching.value = true; }, onFinish: () => { isFetching.value = false; } });
    }, 350);
};
...
const allColumns = computed(() => automationsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }));
...
// ─── Onboarding tour ─────────────────────────────────────────────────────
const tour = useModuleTour({ module: 'automations', steps: () => moduleTourSteps(t, { moduleName: t('automations.plural') }) });
</script>
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="automations" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('automations.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="canManage && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="canManage && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('automation_management.automations.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('automation_management.automations.trash'))">
                ...
                <Tooltip v-if="canManage" :title="$t('automations.new')" data-tour="new">
                    <Link :href="route('automation_management.automations.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('automations.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            >
                ...
                <template #bodyCell="{ column, record, isMobile, text, compact }">
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('automation_management.automations.show', record.id)" class="lead__name lead__link">{{ record.name }}</Link>
                                <span v-if="record.description" class="lead__sub">{{ record.description }}</span>
                                <span v-else-if="record.data_source" class="lead__sub">{{ sourceLabel(record.data_source) }}</span>
                            </div>
                        </div>
                    </template>
