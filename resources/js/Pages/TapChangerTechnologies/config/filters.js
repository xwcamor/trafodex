...
/**
 * Schema de filtros del módulo TapChangerTechnologies (catalogo per-tenant).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const tapChangerTechnologiesFilterFields = (t) => [
    { key: 'name',           label: t('tap_changer_technologies.filter_name'), type: 'tags' },
    { key: 'code',           label: t('tap_changer_technologies.code'),        type: 'text' },
    { key: 'is_active',      label: t('tap_changer_technologies.is_active'),   type: 'select', options: [
...
export const tapChangerTechnologiesEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateTapChangerTechnologiesFilters = (server) => ({
...
export const tapChangerTechnologiesFiltersToQuery = (f) => ({
...
export const tapChangerTechnologiesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('tap_changer_technologies.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('tap_changer_technologies.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('tap_changer_technologies.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
