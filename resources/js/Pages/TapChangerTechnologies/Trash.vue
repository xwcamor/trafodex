...
import TapChangerTechnologiesTrashBulkBar from '@/Components/TapChangerTechnologies/TapChangerTechnologiesTrashBulkBar.vue';
import TapChangerTechnologiesForceDeleteModal from '@/Components/TapChangerTechnologies/TapChangerTechnologiesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { tapChangerTechnologiesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.tap_changer_technologies.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.tap_changer_technologies.restore',
    bulkRestoreRouteName: 'business_management.tap_changer_technologies.bulk_restore',
});
...
const openForceDelete = (tapChangerTechnology) => {
    forceDeleteTarget.value = tapChangerTechnology;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.tap_changer_technologies.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => tapChangerTechnologiesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.tapChangerTechnologies.total === 1 ? t('global.record') : t('global.records');
    return `${props.tapChangerTechnologies.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_technologies.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.tap_changer_technologies.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_technologies.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('tap_changer_technologies.restore_hint')"
            ...
            <Empty
                v-if="tapChangerTechnologies.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <TapChangerTechnologiesTrashBulkBar
        ...
        <TapChangerTechnologiesForceDeleteModal
