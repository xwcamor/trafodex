...
/**
 * Schema de filtros del módulo TapChangerModels (catalogo per-tenant).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const tapChangerModelsFilterFields = (t) => [
    { key: 'name',           label: t('tap_changer_models.filter_name'), type: 'tags' },
    { key: 'code',           label: t('tap_changer_models.code'),        type: 'text' },
    { key: 'is_active',      label: t('tap_changer_models.is_active'),   type: 'select', options: [
...
export const tapChangerModelsEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateTapChangerModelsFilters = (server) => ({
...
export const tapChangerModelsFiltersToQuery = (f) => ({
...
export const tapChangerModelsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('tap_changer_models.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('tap_changer_models.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('tap_changer_models.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
