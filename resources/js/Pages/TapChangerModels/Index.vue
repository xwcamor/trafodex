...
import TapChangerModelsBulkBar from '@/Components/TapChangerModels/TapChangerModelsBulkBar.vue';
import TapChangerModelsBulkDeleteModal from '@/Components/TapChangerModels/TapChangerModelsBulkDeleteModal.vue';
import TapChangerModelsFavoriteCell from '@/Components/TapChangerModels/TapChangerModelsFavoriteCell.vue';
...
// El toolbar inline de TapChangerModels (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    tapChangerModelsFilterFields, tapChangerModelsEmptyFilters, hydrateTapChangerModelsFilters,
    tapChangerModelsFiltersToQuery, tapChangerModelsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { tapChangerModelsTableColumns } from './config/columns';
import { tapChangerModelsExportableColumns, tapChangerModelsExportEndpoints } from './config/exports';
...
const filterFields = computed(() =>
    tapChangerModelsFilterFields(t),
);
...
const {
    ...
    hydrate:       hydrateTapChangerModelsFilters,
    toQuery:       tapChangerModelsFiltersToQuery,
    summary:       tapChangerModelsFiltersSummary,
    empty:         tapChangerModelsEmptyFilters,
    only:          ['tap_changer_models', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.tapChangerModels),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    tapChangerModelsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.tapChangerModels.current_page,
    pageSize: props.tapChangerModels.per_page,
    total:    props.tapChangerModels.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.tap_changer_models.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'tap_changer_models_view_mode';
...
useModuleUndoToast('business_management.tap_changer_models.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('tap_changer_models', 'tap_changer_models');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.tap_changer_models.bulk_set_active',
    bulkDeleteRoute:    'business_management.tap_changer_models.bulk_delete',
    resourceLabel:      t('tap_changer_models.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.tap_changer_models.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => tapChangerModelsExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => tapChangerModelsExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <div class="mi-console mi-console--v2">
            ...
            <span class="mi-colsel-host" aria-hidden="true">
                <ColumnSelector
                    ...
                    storage-key="tap_changer_models"
                />
            </span>
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="tap_changer_models" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('tap_changer_models.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('tap_changer_models.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('tap_changer_models.create') && can('tap_changer_models.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('tap_changer_models.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.tap_changer_models.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.tap_changer_models.trash'))">
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="tapChangerModels.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('tap_changer_models.delete') || can('tap_changer_models.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <TapChangerModelsFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === tapChangerModels.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.tap_changer_models.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <TapChangerModelsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('tap_changer_models.record') : $t('tap_changer_models.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('tap_changer_models.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="tapChangerModels.total ?? 0"
            :total-unfiltered="tapChangerModels.total_unfiltered ?? tapChangerModels.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.tap_changer_models.import')"
            :template-url="route('business_management.tap_changer_models.import_template')"
