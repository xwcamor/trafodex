...
import TapChangerModelsTrashBulkBar from '@/Components/TapChangerModels/TapChangerModelsTrashBulkBar.vue';
import TapChangerModelsForceDeleteModal from '@/Components/TapChangerModels/TapChangerModelsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { tapChangerModelsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.tap_changer_models.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.tap_changer_models.restore',
    bulkRestoreRouteName: 'business_management.tap_changer_models.bulk_restore',
});
...
const openForceDelete = (tapChangerModel) => {
    forceDeleteTarget.value = tapChangerModel;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.tap_changer_models.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => tapChangerModelsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.tapChangerModels.total === 1 ? t('global.record') : t('global.records');
    return `${props.tapChangerModels.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_models.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.tap_changer_models.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_models.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('tap_changer_models.restore_hint')"
            ...
            <Empty
                v-if="tapChangerModels.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <TapChangerModelsTrashBulkBar
        ...
        <TapChangerModelsForceDeleteModal
