...
const submit = () => {
    form.delete(route('business_management.tap_changer_models.deleteSave', props.tapChangerModel.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('tap_changer_models.singular')" />

    <DeletePage
        :back-href="route('business_management.tap_changer_models.index')"
        :title="$t('global.delete') + ' ' + $t('tap_changer_models.record')"
        :subtitle="tapChangerModel.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('tap_changer_models.name')">{{ tapChangerModel.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="tapChangerModel.code" :label="$t('tap_changer_models.code')">
                <code>{{ tapChangerModel.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('tap_changer_models.is_active')">
                <Tag :color="tapChangerModel.is_active ? 'success' : 'error'" :bordered="false">
