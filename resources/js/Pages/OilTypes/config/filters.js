...
/**
 * Schema de filtros del módulo OilTypes (catálogo global de tipos de aceite).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const oilTypesFilterFields = (t) => [
    { key: 'name',           label: t('oil_types.filter_name'), type: 'tags' },
    { key: 'code',           label: t('oil_types.code'),        type: 'text' },
    { key: 'is_active',      label: t('oil_types.is_active'),   type: 'select', options: [
...
export const oilTypesEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateOilTypesFilters = (server) => ({
...
export const oilTypesFiltersToQuery = (f) => ({
...
export const oilTypesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('oil_types.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('oil_types.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('oil_types.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
