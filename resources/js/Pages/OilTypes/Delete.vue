...
const submit = () => {
    form.delete(route('business_management.oil_types.deleteSave', props.oilType.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('oil_types.singular')" />

    <DeletePage
        :back-href="route('business_management.oil_types.index')"
        :title="$t('global.delete') + ' ' + $t('oil_types.record')"
        :subtitle="oilType.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('oil_types.name')">{{ oilType.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="oilType.code" :label="$t('oil_types.code')">
                <code>{{ oilType.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('oil_types.is_active')">
                <Tag :color="oilType.is_active ? 'success' : 'error'" :bordered="false">
