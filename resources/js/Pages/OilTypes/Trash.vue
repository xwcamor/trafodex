...
import OilTypesTrashBulkBar from '@/Components/OilTypes/OilTypesTrashBulkBar.vue';
import OilTypesForceDeleteModal from '@/Components/OilTypes/OilTypesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { oilTypesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.oil_types.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.oil_types.restore',
    bulkRestoreRouteName: 'business_management.oil_types.bulk_restore',
});
...
const openForceDelete = (oilType) => {
    forceDeleteTarget.value = oilType;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.oil_types.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => oilTypesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.oilTypes.total === 1 ? t('global.record') : t('global.records');
    return `${props.oilTypes.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('oil_types.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.oil_types.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('oil_types.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('oil_types.restore_hint')"
            ...
            <Empty
                v-if="oilTypes.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <OilTypesTrashBulkBar
