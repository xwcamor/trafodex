...
import OilTypesBulkBar from '@/Components/OilTypes/OilTypesBulkBar.vue';
import OilTypesBulkDeleteModal from '@/Components/OilTypes/OilTypesBulkDeleteModal.vue';
import OilTypesFavoriteCell from '@/Components/OilTypes/OilTypesFavoriteCell.vue';
...
// El toolbar inline de OilTypes (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    oilTypesFilterFields, oilTypesEmptyFilters, hydrateOilTypesFilters,
    oilTypesFiltersToQuery, oilTypesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { oilTypesTableColumns } from './config/columns';
import { oilTypesExportableColumns, oilTypesExportEndpoints } from './config/exports';
...
const {
    ...
    hydrate:       hydrateOilTypesFilters,
    toQuery:       oilTypesFiltersToQuery,
    summary:       oilTypesFiltersSummary,
    empty:         oilTypesEmptyFilters,
    only:          ['oil_types', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.oilTypes),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    oilTypesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.oilTypes.current_page,
    pageSize: props.oilTypes.per_page,
    total:    props.oilTypes.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.oil_types.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'oil_types_view_mode';
...
useModuleUndoToast('business_management.oil_types.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('oil_types', 'oil_types');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.oil_types.bulk_set_active',
    bulkDeleteRoute:    'business_management.oil_types.bulk_delete',
    resourceLabel:      t('oil_types.records'),
});
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.oil_types.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => oilTypesExportableColumns(t));
const exportEndpoints   = computed(() => oilTypesExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="oil_types" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('oil_types.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('oil_types.create') && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('oil_types.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.oil_types.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.oil_types.trash'))">
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="oilTypes.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('oil_types.delete') || can('oil_types.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <OilTypesFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === oilTypes.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.oil_types.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
                    ...
                    <template v-else-if="column.key === 'rules'">
                        <Tooltip :title="$t('oil_types.has_rules_hint')">
                            <Tag :color="record.has_rules ? 'green' : 'default'" style="margin: 0">
                                {{ record.has_rules ? $t('oil_types.has_rules_yes') : $t('oil_types.has_rules_no') }}
                            </Tag>
                        </Tooltip>
                    </template>
        ...
        <OilTypesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('oil_types.record') : $t('oil_types.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('oil_types.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="oilTypes.total ?? 0"
            :total-unfiltered="oilTypes.total_unfiltered ?? oilTypes.total ?? 0"
        />
