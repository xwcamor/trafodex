...
import { SaveOutlined, UndoOutlined, EditOutlined, UserOutlined } from '@ant-design/icons-vue';
...
<template>
    ...
    <div class="edit-all sap-form">
        ...
        <div v-if="users.total > users.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('users.edit_all_discard')"
            :save-label="$t('users.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('users.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
