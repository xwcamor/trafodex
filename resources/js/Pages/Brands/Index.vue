...
import BrandsBulkBar from '@/Components/Brands/BrandsBulkBar.vue';
import BrandsBulkDeleteModal from '@/Components/Brands/BrandsBulkDeleteModal.vue';
import BrandsFavoriteCell from '@/Components/Brands/BrandsFavoriteCell.vue';
...
// El toolbar inline de Brands (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    brandsFilterFields, brandsEmptyFilters, hydrateBrandsFilters,
    brandsFiltersToQuery, brandsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { brandsTableColumns } from './config/columns';
import { brandsExportableColumns, brandsExportEndpoints } from './config/exports';
...
const {
    ...
    hydrate:       hydrateBrandsFilters,
    toQuery:       brandsFiltersToQuery,
    summary:       brandsFiltersSummary,
    empty:         brandsEmptyFilters,
    only:          ['brands', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.brands),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    brandsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.brands.current_page,
    pageSize: props.brands.per_page,
    total:    props.brands.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.brands.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'brands_view_mode';
...
useModuleUndoToast('business_management.brands.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('brands', 'brands');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.brands.bulk_set_active',
    bulkDeleteRoute:    'business_management.brands.bulk_delete',
    resourceLabel:      t('brands.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.brands.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => brandsExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => brandsExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="brands" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('brands.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('brands.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('brands.create') && can('brands.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('brands.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.brands.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.brands.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('brands.delete') || can('brands.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <BrandsFavoriteCell
                        ...
                        :data-tour="record === brands.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.brands.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <BrandsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('brands.record') : $t('brands.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('brands.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="brands.total ?? 0"
            :total-unfiltered="brands.total_unfiltered ?? brands.total ?? 0"
        />
