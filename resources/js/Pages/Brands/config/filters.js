...
/**
 * Schema de filtros del módulo Brands (catálogo global de marcas).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const brandsFilterFields = (t) => [
    { key: 'name',           label: t('brands.filter_name'), type: 'tags' },
    { key: 'code',           label: t('brands.code'),        type: 'text' },
    { key: 'is_active',      label: t('brands.is_active'),   type: 'select', options: [
...
export const brandsEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateBrandsFilters = (server) => ({
...
export const brandsFiltersToQuery = (f) => ({
...
export const brandsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('brands.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('brands.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('brands.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
