...
import BrandsEditAllTable from '@/Components/Brands/BrandsEditAllTable.vue';
...
/**
 * Edit-All de Brands. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.brands.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.brands.edit_all.update'),
...
<template>
    <Head :title="$t('brands.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.brands.index')"
            :title="$t('global.edit_all') + ' — ' + $t('brands.plural')"
            :subtitle="$t('brands.edit_all_subtitle')"
        >
        ...
        <div v-if="brands.total > brands.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('brands.edit_all_discard')"
            :save-label="$t('brands.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('brands.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
