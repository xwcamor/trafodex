...
import BrandsTrashBulkBar from '@/Components/Brands/BrandsTrashBulkBar.vue';
import BrandsForceDeleteModal from '@/Components/Brands/BrandsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { brandsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.brands.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.brands.restore',
    bulkRestoreRouteName: 'business_management.brands.bulk_restore',
});
...
const openForceDelete = (brand) => {
    forceDeleteTarget.value = brand;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.brands.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => brandsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.brands.total === 1 ? t('global.record') : t('global.records');
    return `${props.brands.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('brands.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.brands.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('brands.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('brands.restore_hint')"
