...
import TapChangerBrandsBulkBar from '@/Components/TapChangerBrands/TapChangerBrandsBulkBar.vue';
import TapChangerBrandsBulkDeleteModal from '@/Components/TapChangerBrands/TapChangerBrandsBulkDeleteModal.vue';
import TapChangerBrandsFavoriteCell from '@/Components/TapChangerBrands/TapChangerBrandsFavoriteCell.vue';
import TapChangerBrandsPageHeader from '@/Components/TapChangerBrands/TapChangerBrandsPageHeader.vue';
import TapChangerBrandsActionsCell from '@/Components/TapChangerBrands/TapChangerBrandsActionsCell.vue';
import TapChangerBrandsEmptyState from '@/Components/TapChangerBrands/TapChangerBrandsEmptyState.vue';
...
// El toolbar inline de TapChangerBrands (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    tapChangerBrandsFilterFields, tapChangerBrandsEmptyFilters, hydrateTapChangerBrandsFilters,
    tapChangerBrandsFiltersToQuery, tapChangerBrandsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { tapChangerBrandsTableColumns } from './config/columns';
import { tapChangerBrandsExportableColumns, tapChangerBrandsExportEndpoints } from './config/exports';
...
const props = defineProps({
    tapChangerBrands:      { type: Object, required: true },
...
const filterFields = computed(() =>
    tapChangerBrandsFilterFields(t),
);
...
const {
    ...
    hydrate:       hydrateTapChangerBrandsFilters,
    toQuery:       tapChangerBrandsFiltersToQuery,
    summary:       tapChangerBrandsFiltersSummary,
    empty:         tapChangerBrandsEmptyFilters,
    only:          ['tap_changer_brands', 'filters'],
    t,
});
...
const clearAll = () => {
    advancedWhere.value = [];
    router.get(
        route('business_management.tap_changer_brands.index'),
        {
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.tapChangerBrands),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    tapChangerBrandsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.tapChangerBrands.current_page,
    pageSize: props.tapChangerBrands.per_page,
    total:    props.tapChangerBrands.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.tap_changer_brands.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'tap_changer_brands_view_mode';
...
useModuleUndoToast('business_management.tap_changer_brands.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('tap_changer_brands', 'tap_changer_brands');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.tap_changer_brands.bulk_set_active',
    bulkDeleteRoute:    'business_management.tap_changer_brands.bulk_delete',
    resourceLabel:      t('tap_changer_brands.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.tap_changer_brands.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => tapChangerBrandsExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => tapChangerBrandsExportEndpoints());
...
const tour = useModuleTour({ module: 'tap_changer_brands', steps: () => moduleTourSteps(t, { moduleName: t('tap_changer_brands.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('tap_changer_brands.create') && router.visit(route('business_management.tap_changer_brands.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.tap_changer_brands.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.tap_changer_brands.delete', record.slug));
</script>

<template>
    <Head :title="$t('tap_changer_brands.plural')" />

    <div class="sap-index">
        <!-- Título (izq) + acciones (Vistas, Exportar, engranaje, Nuevo) a la derecha. -->
        <div class="mi-title" data-tour="module">
            <TapChangerBrandsPageHeader
                :title="$t('tap_changer_brands.plural')"
            />
        </div>
        ...
        <div class="mi-console mi-console--v2">
            ...
            <span class="mi-colsel-host" aria-hidden="true">
                <ColumnSelector
                    ...
                    storage-key="tap_changer_brands"
                />
            </span>
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="tap_changer_brands" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('tap_changer_brands.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('tap_changer_brands.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('tap_changer_brands.create') && can('tap_changer_brands.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('tap_changer_brands.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.tap_changer_brands.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.tap_changer_brands.trash'))">
                ...
                <Tooltip v-if="can('tap_changer_brands.create')" :title="$t('tap_changer_brands.new')" data-tour="new">
                    <Link :href="route('business_management.tap_changer_brands.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('tap_changer_brands.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="tapChangerBrands.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('tap_changer_brands.delete') || can('tap_changer_brands.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <TapChangerBrandsEmptyState
                        :has-filters="hasActiveFilters"
                        :can-create="can('tap_changer_brands.create')"
                        @clear-filters="clearFilters"
                        @open-import="importOpen = true"
                    />
                </template>
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <TapChangerBrandsFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === tapChangerBrands.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.tap_changer_brands.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <TapChangerBrandsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('tap_changer_brands.record') : $t('tap_changer_brands.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('tap_changer_brands.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="tapChangerBrands.total ?? 0"
            :total-unfiltered="tapChangerBrands.total_unfiltered ?? tapChangerBrands.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.tap_changer_brands.import')"
            :template-url="route('business_management.tap_changer_brands.import_template')"
            :resource-label="$t('tap_changer_brands.records')"
        />
