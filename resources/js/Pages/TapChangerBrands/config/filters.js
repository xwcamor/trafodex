...
/**
 * Schema de filtros del módulo TapChangerBrands (catalogo per-tenant).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const tapChangerBrandsFilterFields = (t) => [
    { key: 'name',           label: t('tap_changer_brands.filter_name'), type: 'tags' },
    { key: 'code',           label: t('tap_changer_brands.code'),        type: 'text' },
    { key: 'is_active',      label: t('tap_changer_brands.is_active'),   type: 'select', options: [
...
export const tapChangerBrandsEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateTapChangerBrandsFilters = (server) => ({
...
export const tapChangerBrandsFiltersToQuery = (f) => ({
...
export const tapChangerBrandsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('tap_changer_brands.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('tap_changer_brands.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('tap_changer_brands.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
