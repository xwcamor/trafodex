...
import TapChangerBrandsEditAllTable from '@/Components/TapChangerBrands/TapChangerBrandsEditAllTable.vue';
...
/**
 * Edit-All de TapChangerBrands. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.tapChangerBrands.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.tap_changer_brands.edit_all.update'),
...
const onPageChange = (page, pageSize) => {
    router.get(
        route('business_management.tap_changer_brands.edit_all'),
...
<template>
    <Head :title="$t('tap_changer_brands.edit_all_title')" />
    ...
    <div class="edit-all sap-form">
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('tap_changer_brands.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <TapChangerBrandsEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="tapChangerBrands.total > tapChangerBrands.per_page" class="edit-pagination">
            <Pagination
                :current="tapChangerBrands.current_page"
                :pageSize="tapChangerBrands.per_page"
                :total="tapChangerBrands.total"
        ...
        <EditAllFooter
            :discard-label="$t('tap_changer_brands.edit_all_discard')"
            :save-label="$t('tap_changer_brands.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('tap_changer_brands.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
