...
import TapChangerBrandsTrashBulkBar from '@/Components/TapChangerBrands/TapChangerBrandsTrashBulkBar.vue';
import TapChangerBrandsForceDeleteModal from '@/Components/TapChangerBrands/TapChangerBrandsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { tapChangerBrandsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.tap_changer_brands.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.tap_changer_brands.restore',
    bulkRestoreRouteName: 'business_management.tap_changer_brands.bulk_restore',
});
...
const openForceDelete = (tapChangerBrand) => {
    forceDeleteTarget.value = tapChangerBrand;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.tap_changer_brands.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => tapChangerBrandsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.tapChangerBrands.total === 1 ? t('global.record') : t('global.records');
    return `${props.tapChangerBrands.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_brands.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.tap_changer_brands.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('tap_changer_brands.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('tap_changer_brands.restore_hint')"
            ...
            <Empty
                v-if="tapChangerBrands.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <TapChangerBrandsTrashBulkBar
        ...
        <TapChangerBrandsForceDeleteModal
