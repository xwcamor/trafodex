...
import TransformerTypesTrashBulkBar from '@/Components/TransformerTypes/TransformerTypesTrashBulkBar.vue';
import TransformerTypesForceDeleteModal from '@/Components/TransformerTypes/TransformerTypesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { transformerTypesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.transformer_types.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.transformer_types.restore',
    bulkRestoreRouteName: 'business_management.transformer_types.bulk_restore',
});
...
const openForceDelete = (transformerType) => {
    forceDeleteTarget.value = transformerType;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.transformer_types.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => transformerTypesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.transformerTypes.total === 1 ? t('global.record') : t('global.records');
    return `${props.transformerTypes.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('transformer_types.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.transformer_types.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('transformer_types.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('transformer_types.restore_hint')"
            ...
            <Empty
                v-if="transformerTypes.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <TransformerTypesTrashBulkBar
        ...
        <TransformerTypesForceDeleteModal
