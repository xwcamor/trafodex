...
import TransformerTypesEditAllTable from '@/Components/TransformerTypes/TransformerTypesEditAllTable.vue';
...
/**
 * Edit-All de TransformerTypes. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.transformerTypes.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.transformer_types.edit_all.update'),
...
const onPageChange = (page, pageSize) => {
    router.get(
        route('business_management.transformer_types.edit_all'),
...
<template>
    <Head :title="$t('transformer_types.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.transformer_types.index')"
            :title="$t('global.edit_all') + ' — ' + $t('transformer_types.plural')"
            :subtitle="$t('transformer_types.edit_all_subtitle')"
        >
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('transformer_types.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <TransformerTypesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="transformerTypes.total > transformerTypes.per_page" class="edit-pagination">
            <Pagination
                :current="transformerTypes.current_page"
                :pageSize="transformerTypes.per_page"
                :total="transformerTypes.total"
        ...
        <EditAllFooter
            :discard-label="$t('transformer_types.edit_all_discard')"
            :save-label="$t('transformer_types.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('transformer_types.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
