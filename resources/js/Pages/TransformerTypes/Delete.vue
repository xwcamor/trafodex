...
const submit = () => {
    form.delete(route('business_management.transformer_types.deleteSave', props.transformerType.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('transformer_types.singular')" />

    <DeletePage
        :back-href="route('business_management.transformer_types.index')"
        :title="$t('global.delete') + ' ' + $t('transformer_types.record')"
        :subtitle="transformerType.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('transformer_types.name')">{{ transformerType.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="transformerType.code" :label="$t('transformer_types.code')">
                <code>{{ transformerType.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('transformer_types.is_active')">
                <Tag :color="transformerType.is_active ? 'success' : 'error'" :bordered="false">
