...
/**
 * Schema de filtros del módulo TransformerTypes (catálogo global de tipos de transformador).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const transformerTypesFilterFields = (t) => [
    { key: 'name',           label: t('transformer_types.filter_name'), type: 'tags' },
    { key: 'code',           label: t('transformer_types.code'),        type: 'text' },
    { key: 'is_active',      label: t('transformer_types.is_active'),   type: 'select', options: [
...
export const transformerTypesEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateTransformerTypesFilters = (server) => ({
...
export const transformerTypesFiltersToQuery = (f) => ({
...
export const transformerTypesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('transformer_types.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('transformer_types.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('transformer_types.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
