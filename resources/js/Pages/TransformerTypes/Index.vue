...
import TransformerTypesBulkBar from '@/Components/TransformerTypes/TransformerTypesBulkBar.vue';
import TransformerTypesBulkDeleteModal from '@/Components/TransformerTypes/TransformerTypesBulkDeleteModal.vue';
import TransformerTypesFavoriteCell from '@/Components/TransformerTypes/TransformerTypesFavoriteCell.vue';
import TransformerTypesPageHeader from '@/Components/TransformerTypes/TransformerTypesPageHeader.vue';
import TransformerTypesActionsCell from '@/Components/TransformerTypes/TransformerTypesActionsCell.vue';
import TransformerTypesEmptyState from '@/Components/TransformerTypes/TransformerTypesEmptyState.vue';
...
// El toolbar inline de TransformerTypes (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    transformerTypesFilterFields, transformerTypesEmptyFilters, hydrateTransformerTypesFilters,
    transformerTypesFiltersToQuery, transformerTypesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { transformerTypesTableColumns } from './config/columns';
import { transformerTypesExportableColumns, transformerTypesExportEndpoints } from './config/exports';
...
const props = defineProps({
    transformerTypes:      { type: Object, required: true },
...
const filterFields = computed(() =>
    transformerTypesFilterFields(t),
);
...
const {
    ...
    hydrate:       hydrateTransformerTypesFilters,
    toQuery:       transformerTypesFiltersToQuery,
    summary:       transformerTypesFiltersSummary,
    empty:         transformerTypesEmptyFilters,
    only:          ['transformer_types', 'filters'],
    t,
});
...
const clearAll = () => {
    advancedWhere.value = [];
    router.get(
        route('business_management.transformer_types.index'),
        {
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.transformerTypes),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    transformerTypesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.transformerTypes.current_page,
    pageSize: props.transformerTypes.per_page,
    total:    props.transformerTypes.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.transformer_types.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'transformer_types_view_mode';
...
useModuleUndoToast('business_management.transformer_types.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('transformer_types', 'transformer_types');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.transformer_types.bulk_set_active',
    bulkDeleteRoute:    'business_management.transformer_types.bulk_delete',
    resourceLabel:      t('transformer_types.records'),
});
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.transformer_types.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => transformerTypesExportableColumns(t));
const exportEndpoints   = computed(() => transformerTypesExportEndpoints());
...
const tour = useModuleTour({ module: 'transformer_types', steps: () => moduleTourSteps(t, { moduleName: t('transformer_types.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('transformer_types.create') && router.visit(route('business_management.transformer_types.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.transformer_types.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.transformer_types.delete', record.slug));
</script>

<template>
    <Head :title="$t('transformer_types.plural')" />

    <div class="sap-index">
        <!-- Título (izq) + acciones (Vistas, Exportar, engranaje, Nuevo) a la derecha. -->
        <div class="mi-title" data-tour="module">
            <TransformerTypesPageHeader
                :title="$t('transformer_types.plural')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="transformer_types" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('transformer_types.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('transformer_types.create') && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('transformer_types.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.transformer_types.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.transformer_types.trash'))">
                ...
                <Tooltip v-if="can('transformer_types.create')" :title="$t('transformer_types.new')" data-tour="new">
                    <Link :href="route('business_management.transformer_types.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('transformer_types.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="transformerTypes.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('transformer_types.delete') || can('transformer_types.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <TransformerTypesEmptyState
                        :has-filters="hasActiveFilters"
                        :can-create="can('transformer_types.create')"
                        @clear-filters="clearFilters"
                        @open-import="importOpen = true"
                    />
                </template>
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <TransformerTypesFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === transformerTypes.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.transformer_types.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
                    ...
                    <template v-else-if="column.key === 'shape'">
                        <span class="shape-cell">
                            <TransformerGlyph :shape="record.shape || 'tank'" :animated="false" />
                            {{ $t('transformer_types.shape_' + (record.shape || 'tank')) }}
                        </span>
                    </template>
        ...
        <TransformerTypesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('transformer_types.record') : $t('transformer_types.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('transformer_types.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="transformerTypes.total ?? 0"
            :total-unfiltered="transformerTypes.total_unfiltered ?? transformerTypes.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.transformer_types.import')"
            :template-url="route('business_management.transformer_types.import_template')"
            :resource-label="$t('transformer_types.records')"
        />
    </div>
</template>
...
.shape-cell { display: inline-flex; align-items: center; gap: 8px; }
.shape-cell > :first-child { width: 30px; height: 30px; flex-shrink: 0; }
