...
export const tenantsFilterFields = (t, opts = {}) => {
    // planOptions viene del controller (Plan::publicOptions(), DB-driven).
    // El fallback solo aplica si el componente no lo inyecta — array vacio
    // para no hardcodear slugs que pueden no existir.
    const planOptions = opts.planOptions ?? [];
    return [
        { key: 'name',           label: t('tenants.name'),     type: 'tags' },
        { key: 'plan',           label: t('tenants.plan'),     type: 'multiselect', options: planOptions },
        { key: 'is_active',      label: t('tenants.is_active'), type: 'select', options: [
...
export const tenantsEmptyFilters = () => ({
    name: [],
    plan: [],
    is_active: null,
    created_at: null,
...
export const hydrateTenantsFilters = (sf) => ({
    name:       Array.isArray(sf.name) ? sf.name : [],
    plan:       Array.isArray(sf.plan) ? sf.plan : (sf.plan ? [sf.plan] : []),
...
export const tenantsFiltersToQuery = (f, sf) => ({
    name:           f.name?.length ? f.name : undefined,
    plan:           f.plan?.length ? f.plan : undefined,
...
export const tenantsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('tenants.name')}: ${f.name.join(', ')}`);
    if (f.plan?.length)        parts.push(`${t('tenants.plan')}: ${f.plan.map(p => p.toUpperCase()).join(', ')}`);
    if (f.is_active === true)  parts.push(`${t('tenants.is_active')}: ${t('global.active')}`);
    if (f.is_active === false) parts.push(`${t('tenants.is_active')}: ${t('global.inactive')}`);
...
export const deserializeSavedFilters = (f = {}) => ({
    ...
    plan:       Array.isArray(f.plan) ? f.plan : (f.plan ? [f.plan] : []),
    is_active:  f.is_active ?? null,
