...
import { SaveOutlined, UndoOutlined, EditOutlined, BankOutlined } from '@ant-design/icons-vue';
...
<template>
    ...
    <div class="edit-all sap-form">
        ...
        <div v-if="tenants.total > tenants.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('tenants.edit_all_discard')"
            :save-label="$t('tenants.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('tenants.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
