...
const submit = () => {
    form.delete(route('business_management.laboratories.deleteSave', props.laboratory.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('laboratories.singular')" />

    <DeletePage
        :back-href="route('business_management.laboratories.index')"
        :title="$t('global.delete') + ' ' + $t('laboratories.record')"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('laboratories.name')">{{ laboratory.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="laboratory.code" :label="$t('laboratories.code')">
                <code>{{ laboratory.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('laboratories.is_active')">
                <Tag :color="laboratory.is_active ? 'success' : 'error'" :bordered="false">
