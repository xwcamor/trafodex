...
import LaboratoriesBulkBar from '@/Components/Laboratories/LaboratoriesBulkBar.vue';
import LaboratoriesBulkDeleteModal from '@/Components/Laboratories/LaboratoriesBulkDeleteModal.vue';
import LaboratoriesFavoriteCell from '@/Components/Laboratories/LaboratoriesFavoriteCell.vue';
...
// El toolbar inline de Laboratories (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    laboratoriesFilterFields, laboratoriesEmptyFilters, hydrateLaboratoriesFilters,
    laboratoriesFiltersToQuery, laboratoriesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { laboratoriesTableColumns } from './config/columns';
import { laboratoriesExportableColumns, laboratoriesExportEndpoints } from './config/exports';
...
const {
    ...
    hydrate:       hydrateLaboratoriesFilters,
    toQuery:       laboratoriesFiltersToQuery,
    summary:       laboratoriesFiltersSummary,
    empty:         laboratoriesEmptyFilters,
    only:          ['laboratories', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.laboratories),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    laboratoriesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.laboratories.current_page,
    pageSize: props.laboratories.per_page,
    total:    props.laboratories.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.laboratories.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'laboratories_view_mode';
...
useModuleUndoToast('business_management.laboratories.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('laboratories', 'laboratories');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.laboratories.bulk_set_active',
    bulkDeleteRoute:    'business_management.laboratories.bulk_delete',
    resourceLabel:      t('laboratories.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.laboratories.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => laboratoriesExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => laboratoriesExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="laboratories" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('laboratories.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('laboratories.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('laboratories.create') && can('laboratories.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('laboratories.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.laboratories.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.laboratories.trash'))">
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('laboratories.delete') || can('laboratories.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <LaboratoriesFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === laboratories.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.laboratories.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <LaboratoriesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('laboratories.record') : $t('laboratories.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('laboratories.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="laboratories.total ?? 0"
            :total-unfiltered="laboratories.total_unfiltered ?? laboratories.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.laboratories.import')"
            :template-url="route('business_management.laboratories.import_template')"
