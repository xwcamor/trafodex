...
import LaboratoriesEditAllTable from '@/Components/Laboratories/LaboratoriesEditAllTable.vue';
...
/**
 * Edit-All de Laboratories. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.laboratories.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.laboratories.edit_all.update'),
...
<template>
    <Head :title="$t('laboratories.edit_all_title')" />
    ...
    <div class="edit-all sap-form">
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('laboratories.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <LaboratoriesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="laboratories.total > laboratories.per_page" class="edit-pagination">
            <Pagination
                :current="laboratories.current_page"
                :pageSize="laboratories.per_page"
        ...
        <EditAllFooter
            :discard-label="$t('laboratories.edit_all_discard')"
            :save-label="$t('laboratories.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('laboratories.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
