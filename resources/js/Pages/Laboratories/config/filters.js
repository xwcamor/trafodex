...
/**
 * Schema de filtros del módulo Laboratories (catálogo de laboratorios, per-tenant).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const laboratoriesFilterFields = (t) => [
    { key: 'name',           label: t('laboratories.filter_name'), type: 'tags' },
    { key: 'code',           label: t('laboratories.code'),        type: 'text' },
    { key: 'is_active',      label: t('laboratories.is_active'),   type: 'select', options: [
...
export const laboratoriesEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateLaboratoriesFilters = (server) => ({
...
export const laboratoriesFiltersToQuery = (f) => ({
...
export const laboratoriesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('laboratories.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('laboratories.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('laboratories.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
