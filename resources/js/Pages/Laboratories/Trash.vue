...
import LaboratoriesTrashBulkBar from '@/Components/Laboratories/LaboratoriesTrashBulkBar.vue';
import LaboratoriesForceDeleteModal from '@/Components/Laboratories/LaboratoriesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { laboratoriesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.laboratories.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.laboratories.restore',
    bulkRestoreRouteName: 'business_management.laboratories.bulk_restore',
});
...
const openForceDelete = (laboratory) => {
    forceDeleteTarget.value = laboratory;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.laboratories.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => laboratoriesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.laboratories.total === 1 ? t('global.record') : t('global.records');
    return `${props.laboratories.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('laboratories.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.laboratories.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('laboratories.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('laboratories.restore_hint')"
            ...
            <Empty
                v-if="laboratories.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <LaboratoriesTrashBulkBar
