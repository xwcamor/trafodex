...
<template>
    ...
    <DeletePage
        ...
        @submit="submit"
    >
        <!-- Registros dependientes (jerarquía): advertencia propia del módulo. -->
        <template #warning>
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('customers.name')">{{ customer.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="customer.cod" :label="$t('customers.cod')">
                <code>{{ customer.cod }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow v-if="customer.country" :label="$t('customers.country')">
                {{ customer.country.iso_code }} · {{ customer.country.name }}
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('customers.is_active')">
