...
import { Card, Tag, Tooltip, Button, Badge, Dropdown, Menu, MenuItem, Input, Drawer } from 'ant-design-vue';
import {
    LockOutlined, PlusOutlined, EditOutlined, InboxOutlined,
...
const applyAdvancedFilters = (clauses) => {
    ...
    router.get(route('system_management.settings.index'), data, { preserveScroll: true, preserveState: true, onStart: () => { isFetching.value = true; }, onFinish: () => { isFetching.value = false; } });
};
...
const applySavedViewState = (clauses, meta) => {
    ...
    if (clauses.length > 0) data.advanced_where = JSON.stringify(clauses);
    router.get(route('system_management.settings.index'), data, {
...
// ─── Columns ─────────────────────────────────────────────────────────────
const allColumns = computed(() => settingsTableColumns(t, { isMobile: isMobileScreen.value }));
...
const VIEW_KEY = 'settings_view_mode';
...
// ─── Onboarding tour ─────────────────────────────────────────────────────
const tour = useModuleTour({ module: 'settings', steps: () => moduleTourSteps(t, { moduleName: t('settings.plural') }) });
</script>
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="settings" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('settings.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('settings.create') && canUsePlanFeature('imports')" key="import" @click="openImport">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('settings.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('system_management.settings.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('system_management.settings.trash'))">
                ...
                <Tooltip v-if="can('settings.create')" :title="$t('settings.new')" data-tour="new">
                    <Link :href="route('system_management.settings.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('settings.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
