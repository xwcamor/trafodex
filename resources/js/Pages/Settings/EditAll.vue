...
import { SaveOutlined, UndoOutlined, EditOutlined, SettingOutlined } from '@ant-design/icons-vue';
...
<template>
    ...
    <div class="edit-all sap-form">
        ...
        <div v-if="settings.total > settings.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('settings.edit_all_discard')"
            :save-label="$t('settings.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('settings.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
