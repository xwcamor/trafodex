 ...
 *  1) Confirmacion de serial (deben tipear el serial exacto).
...
const okDisabled = (target) => !target
    || form.value.name_confirmation !== (target.serial ?? '')
...
<template>
    ...
    >
        ...
        <p class="force-msg">
            {{ $t('global.force_delete_name_prompt', { name: target?.serial }) }}
        </p>
        <Input
            v-model:value="form.name_confirmation"
            :placeholder="target?.serial"
