<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de Transformers. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('transformers.serial') }}</th>
                <th class="col-cod">{{ $t('transformers.tag') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('transformers.edit_all_no_results') }}
    </div>
</template>
