...
// Registro BLOQUEADO (Lockable): se ocultan editar/eliminar (el server también
// los rechaza). Desbloquear se hace desde la ficha.
...
const onMenu = ({ key }) => {
    if (key === 'view')           router.visit(route('business_management.transformers.show', props.record.slug));
    else if (key === 'edit')      router.visit(route('business_management.transformers.edit', props.record.slug));
...
<template>
    ...
    <!-- Mobile: Abrir (ficha) -> Editar -> Duplicar -> Eliminar.
         La serie navega a la ficha (Show). -->
    ...
    <!-- Desktop: Abrir (ficha) + Editar + Duplicar + Eliminar.
         La serie navega a la ficha (Show). -->
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit && !isLocked" :title="t('global.edit')">
            <Link :href="route('business_management.transformers.edit', record.slug)">
