<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de TapChangerModels. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('tap_changer_models.table_headers.editable_name') }}</th>
                <th class="col-cod">{{ $t('tap_changer_models.code') }}</th>
                <th class="col-status">{{ $t('tap_changer_models.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('tap_changer_models.edit_all_no_results') }}
    </div>
</template>
