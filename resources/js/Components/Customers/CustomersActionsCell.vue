...
const onMenu = ({ key }) => {
    if (key === 'view')       router.visit(route('business_management.customers.show', props.record.slug));
    else if (key === 'edit')  router.visit(route('business_management.customers.edit', props.record.slug));
...
<template>
    ...
    <!-- Mobile: Abrir (ficha) -> Editar -> Duplicar -> Eliminar.
         El nombre y la card también navegan a la ficha (Show). -->
