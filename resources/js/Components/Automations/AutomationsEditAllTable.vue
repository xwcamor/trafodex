...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('automations.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('automations.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('automations.edit_all_no_results') }}
    </div>
</template>
