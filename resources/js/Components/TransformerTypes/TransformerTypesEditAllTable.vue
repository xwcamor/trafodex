<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de TransformerTypes. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('transformer_types.table_headers.editable_name') }}</th>
                <th class="col-cod">{{ $t('transformer_types.code') }}</th>
                <th class="col-status">{{ $t('transformer_types.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('transformer_types.edit_all_no_results') }}
    </div>
</template>
