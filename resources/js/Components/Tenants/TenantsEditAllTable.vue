...
// Edit-all de tenants: solo name + is_active. El plan no es columna del
// tenant — se gestiona por suscripciones, no por edición batch.
    ...
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('tenants.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('tenants.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
