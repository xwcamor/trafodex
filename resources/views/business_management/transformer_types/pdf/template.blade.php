...
<body>
    ...
    @else
        <table class="data">
            ...
            <tbody>
                @foreach ($transformer_types as $tt)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')         {{ $tt->id }} @break
                                    @case('name')       {{ $tt->name }} @break
                                    @case('code')       {{ $tt->code ?? '' }} @break
                                    @case('sort_order') {{ $tt->sort_order ?? '' }} @break
                                    @case('is_active')
                                        <span class="{{ $tt->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $tt->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $tt->slug }} @break
                                    @case('created_at') {{ $tt->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $tt->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $tt->creator->name ?? '—' }} @break
                                    @default {{ $tt->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
