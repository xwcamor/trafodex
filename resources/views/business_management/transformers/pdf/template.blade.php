...
<body>
    ...
    @php
        // Definición única de columnas (compartida con Excel/CSV/Word). El job la
        // pasa; si no, se deriva aquí para que el template sea auto-suficiente.
        $columnDefs = $columnDefs ?? \App\Support\Transformers\TransformerExportColumns::definitions($tz ?? config('app.timezone'));
        $columns    = array_values(array_filter($columns, fn ($c) => isset($columnDefs[$c])));
        $headings   = collect($columnDefs)->map(fn ($d) => $d['heading'])->all();
    @endphp
    ...
    @else
        <table class="data">
            ...
            <tbody>
                @foreach ($transformers as $tr)
                    <tr>
                        @foreach ($columns as $col)
                            <td>{{ $columnDefs[$col]['value']($tr) ?? '' }}</td>
                        @endforeach
                    </tr>
                @endforeach
            </tbody>
