...
<body>
    ...
    @else
        <table class="data">
            ...
            <tbody>
                @foreach ($laboratories as $tc)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')         {{ $tc->id }} @break
                                    @case('name')       {{ $tc->name }} @break
                                    @case('code')       {{ $tc->code ?? '' }} @break
                                    @case('sort_order') {{ $tc->sort_order ?? '' }} @break
                                    @case('is_active')
                                        <span class="{{ $tc->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $tc->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $tc->slug }} @break
                                    @case('created_at') {{ $tc->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $tc->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $tc->creator->name ?? '—' }} @break
                                    @default {{ $tc->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
