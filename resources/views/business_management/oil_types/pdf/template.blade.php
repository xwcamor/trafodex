...
<body>
    ...
    @else
        <table class="data">
            ...
            <tbody>
                @foreach ($oil_types as $ot)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')         {{ $ot->id }} @break
                                    @case('name')       {{ $ot->name }} @break
                                    @case('code')       {{ $ot->code ?? '' }} @break
                                    @case('sort_order') {{ $ot->sort_order ?? '' }} @break
                                    @case('is_active')
                                        <span class="{{ $ot->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $ot->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $ot->slug }} @break
                                    @case('created_at') {{ $ot->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $ot->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $ot->creator->name ?? '—' }} @break
                                    @default {{ $ot->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
