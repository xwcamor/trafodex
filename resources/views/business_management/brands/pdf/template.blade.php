                        ...
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    ...
                                    @case('code')       {{ $b->code ?? '' }} @break
                                    @case('sort_order') {{ $b->sort_order ?? '' }} @break
                                    @case('is_active')
                                        <span class="{{ $b->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $b->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $b->slug }} @break
                                    @case('created_at') {{ $b->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $b->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $b->creator->name ?? '—' }} @break
                                    @default {{ $b->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
