                ...
                @foreach ($system_modules as $system_module)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')        {{ $system_module->id }} @break
                                    @case('name')      {{ $system_module->name }} @break
                                    @case('is_active')
                                        <span class="{{ $system_module->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $system_module->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $system_module->slug }} @break
                                    @case('created_at') {{ $system_module->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $system_module->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $system_module->creator->name ?? '—' }} @break
