                        ...
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    ...
                                    @case('name')       {{ $country->name }} @break
                                    @case('iso_code')   {{ $country->iso_code }} @break
                                    @case('currency')   {{ $country->currency }} @break
                                    @case('timezone')   {{ $country->timezone }} @break
                                    @case('region')     {{ $country->region->name ?? '—' }} @break
                                    @case('default_locale') {{ $country->defaultLocale->code ?? '—' }} @break
                                    @case('is_active')
                                        <span class="{{ $country->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $country->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $country->slug }} @break
                                    @case('created_at') {{ $country->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $country->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $country->creator->name ?? '—' }} @break
                                    @default {{ $country->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
