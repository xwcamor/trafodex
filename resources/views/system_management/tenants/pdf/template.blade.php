                ...
                @foreach ($tenants as $tenant)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')        {{ $tenant->id }} @break
                                    @case('name')      {{ $tenant->name }} @break
                                    @case('plan')      {{ strtoupper($tenant->currentPlan()) }} @break
                                    @case('type')      {{ ucfirst((string) ($tenant->type ?? 'business')) }} @break
                                    @case('is_active')
                                        <span class="{{ $tenant->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $tenant->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $tenant->slug }} @break
                                    @case('created_at') {{ $tenant->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $tenant->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $tenant->creator->name ?? '—' }} @break
                                    @default {{ $tenant->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
