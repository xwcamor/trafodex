                ...
                @foreach ($settings as $setting)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')        {{ $setting->id }} @break
                                    @case('name')      {{ $setting->name }} @break
                                    @case('is_active')
                                        <span class="{{ $setting->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $setting->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $setting->slug }} @break
                                    @case('created_at') {{ $setting->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $setting->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $setting->creator->name ?? '—' }} @break
                                    @default {{ $setting->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
