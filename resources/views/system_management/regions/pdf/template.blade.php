                ...
                @foreach ($regions as $region)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')        {{ $region->id }} @break
                                    @case('name')      {{ $region->name }} @break
                                    @case('is_active')
                                        <span class="{{ $region->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $region->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $region->slug }} @break
                                    @case('created_at') {{ $region->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $region->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $region->creator->name ?? '—' }} @break
                                    @default {{ $region->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
