                ...
                @foreach ($users as $user)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')        {{ $user->id }} @break
                                    @case('name')      {{ $user->name }} @break
                                    @case('email')     {{ $user->email }} @break
                                    @case('role')      {{ $user->roles->first()?->name ?? '—' }} @break
                                    @case('tenant')    {{ $user->tenant?->name ?? '—' }} @break
                                    @case('is_active')
                                        <span class="{{ $user->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $user->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $user->slug }} @break
                                    @case('created_at') {{ $user->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $user->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $user->creator->name ?? '—' }} @break
                                    @case('photo')      {{ $user->photo ?? '' }} @break
                                    @default {{ $user->{$col} ?? '' }}
                                @endswitch
                            </td>
                        @endforeach
