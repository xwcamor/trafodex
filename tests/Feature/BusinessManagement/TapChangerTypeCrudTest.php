...
class TapChangerTypeCrudTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        foreach (['tap_changer_types.view', 'tap_changer_types.create', 'tap_changer_types.edit', 'tap_changer_types.delete', 'tap_changer_types.show'] as $perm) {
            Permission::firstOrCreate(['name' => $perm, 'guard_name' => 'web']);
        }
        // Conmutador (tap changer) es catálogo interno del motor: SOLO super (las
        // rutas van dentro de un grupo role:super). super bypassa permisos vía Gate::before.
    ...
    {
        $response = $this->post(route('business_management.tap_changer_types.store'), [
            'name' => 'Éster Natural', 'code' => 'ester_natural', 'is_active' => true,
        ]);

        $response->assertRedirect();
        $response->assertSessionHasNoErrors();
        $this->assertDatabaseHas('tap_changer_types', ['name' => 'Éster Natural', 'code' => 'ester_natural']);
    }

    public function test_admin_can_update_brand(): void
    {
        $oil = TapChangerType::create(['name' => 'Prueba', 'code' => 'prueba', 'is_active' => true]);

        $response = $this->put(route('business_management.tap_changer_types.update', $oil->slug), [
            'name' => 'Prueba Editada', 'code' => 'prueba', 'is_active' => false,
        ]);

        $response->assertRedirect();
        $response->assertSessionHasNoErrors();
        $this->assertDatabaseHas('tap_changer_types', ['id' => $oil->id, 'name' => 'Prueba Editada', 'is_active' => false]);
    }

    public function test_name_is_unique_globally(): void
    {
        TapChangerType::create(['name' => 'Mineral', 'code' => 'mineral', 'is_active' => true]);
    ...
    {
        TapChangerType::create(['name' => 'Mineral', 'code' => 'mineral', 'is_active' => true]);
        $other = TapChangerType::create(['name' => 'Silicona', 'code' => 'silicona', 'is_active' => true]);

        $response = $this->put(route('business_management.tap_changer_types.update', $other->slug), [
    ...
    public function test_import_crea_y_deduplica_global_sin_tenant(): void
    {
        ...
        $importer = new \App\Imports\BusinessManagement\TapChangerTypes\TapChangerTypesImport('update_or_create', false);
        ...
        $this->assertDatabaseHas('tap_changer_types', ['name' => 'Girasol']);
        // No existe columna tenant_id: si el import la tocara, esto reventaria.
        $this->assertCount(2, TapChangerType::all());
    }
    ...
    public function test_import_maneja_code_y_deduplica_por_code(): void
    {
        ...
        $this->assertDatabaseHas('tap_changer_types', ['name' => 'Silicona', 'code' => 'silicona']);
        $this->assertDatabaseMissing('tap_changer_types', ['name' => 'Otro']);
    }
}
