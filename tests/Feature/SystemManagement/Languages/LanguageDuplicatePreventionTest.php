 ...
 */
class LanguageDuplicatePreventionTest extends LanguageTestCase
{
    // ─── FORM-LEVEL ─────────────────────────────────────────────────────────
    ...
    public function test_create_blocks_exact_name_duplicate(): void
    {
        ...
        Language::factory()->named('América del Sur')->create();

        $response = $this->post(route('system_management.languages.store'), [
            'name' => 'América del Sur', 'iso_code' => 'as',
        ]);
    ...
    public function test_create_blocks_case_insensitive_duplicate(): void
    {
        ...
        $this->actingAsSuperAdmin();
        Language::factory()->named('Europa')->create();

        $response = $this->post(route('system_management.languages.store'), [
            'name' => 'EUROPA', 'iso_code' => 'eu',
        ]);
    ...
    public function test_create_blocks_accent_insensitive_duplicate(): void
    {
        ...
        $response = $this->post(route('system_management.languages.store'), [
            'name' => 'America del Sur', 'iso_code' => 'as',  // sin tilde
        ]);
    ...
    public function test_create_allows_reuse_of_soft_deleted_name(): void
    {
        ...
        $this->actingAsSuperAdmin();
        Language::factory()->named('Antártida')->trashed()->create();

        $response = $this->post(route('system_management.languages.store'), [
            'name' => 'Antártida', 'iso_code' => 'an',
        ]);

        $response->assertSessionHasNoErrors();
        // 1 active + 1 soft-deleted (factory state)
        $this->assertSame(1, Language::count());
        $this->assertSame(2, Language::withTrashed()->count());
    }

    public function test_update_can_save_same_name_for_same_record(): void
    {
        $this->actingAsSuperAdmin();
        $language = Language::factory()->named('Europa')->create();

        $response = $this->put(
            route('system_management.languages.update', $language->slug),
            ['name' => 'Europa', 'iso_code' => $language->iso_code, 'is_active' => true],
        );
    ...
    public function test_update_blocks_duplicate_against_other_record(): void
    {
        $this->actingAsSuperAdmin();
        Language::factory()->named('Europa')->create();
        $other = Language::factory()->named('Asia')->create();

        $response = $this->put(
            route('system_management.languages.update', $other->slug),
            ['name' => 'Europa', 'iso_code' => 'eu', 'is_active' => true],
        );
    ...
    public function test_import_dry_run_flags_in_file_duplicates(): void
    {
        ...
        $csv = $this->csv([
            ...
            ['América del Sur', 'sa', '1'],
            ['América del Sur', 'xa', '1'],   // duplicate of row 2
            ['Europa',          'eu', '1'],
        ]);

        $response = $this->post(
            route('system_management.languages.import'),
    ...
    public function test_import_commit_skips_existing_in_create_only_mode(): void
    {
        ...
        $csv = $this->csv([
            ['name', 'iso_code', 'is_active'],
            ['Europa', 'eu', '1'],   // already exists → skip
            ['Asia',   'as', '1'],   // new → create
        ]);
    ...
    {
        $this->actingAsSuperAdmin();
        $europa = Language::factory()->named('Europa')->create(['is_active' => false]);

        $csv = $this->csv([
            ['name', 'iso_code', 'is_active'],
            ['Europa', 'eu', '1'],   // exists → update is_active to true
        ]);
    ...
    public function test_import_dry_run_does_not_persist_changes(): void
    {
        ...
        $this->assertSame(0, Language::count(), 'Dry-run must not persist any rows.');
    }
    ...
    protected function csv(array $rows): UploadedFile
    {
        ...
        return new UploadedFile($tmp, 'languages.csv', 'text/csv', null, true);
    }
}
