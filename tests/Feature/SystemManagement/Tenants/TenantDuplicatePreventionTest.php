...
/**
 * Locks down the 3-layer duplicate prevention for Tenants:
 *
 *   1. Form submission (StoreRequest / UpdateRequest via UniqueNormalizedName Rule)
 *   2. Excel/CSV import (TenantsImport: in-file dedup + DB lookup)
 *   3. DB-level partial unique index (final safety net)
 *
 * If you clone Tenants to build a new module, copy this file and rename.
 */
class TenantDuplicatePreventionTest extends TenantTestCase
{
    // ─── FORM-LEVEL ─────────────────────────────────────────────────────────
    ...
    public function test_create_blocks_exact_name_duplicate(): void
    {
        ...
        Tenant::factory()->named('América del Sur')->create();
    ...
    public function test_create_blocks_case_insensitive_duplicate(): void
    {
        ...
        $this->actingAsSuperAdmin();
        Tenant::factory()->named('Europa')->create();
    ...
    public function test_create_allows_reuse_of_soft_deleted_name(): void
    {
        ...
        $this->actingAsSuperAdmin();
        Tenant::factory()->named('Antártida')->trashed()->create();
        ...
        $this->assertSame(1, Tenant::count());
        $this->assertSame(2, Tenant::withTrashed()->count());
    }

    public function test_update_can_save_same_name_for_same_record(): void
    {
        $this->actingAsSuperAdmin();
        $tenant = Tenant::factory()->named('Europa')->create();

        $response = $this->put(route('system_management.tenants.update', $tenant->slug), $this->validTenantData(['name' => 'Europa', 'is_active' => true]));
    ...
    public function test_update_blocks_duplicate_against_other_record(): void
    {
        $this->actingAsSuperAdmin();
        Tenant::factory()->named('Europa')->create();
        $other = Tenant::factory()->named('Asia')->create();

        $response = $this->put(route('system_management.tenants.update', $other->slug), $this->validTenantData(['name' => 'Europa', 'is_active' => true]));
    ...
    public function test_import_dry_run_flags_in_file_duplicates(): void
    {
        ...
        $response = $this->post(
            route('system_management.tenants.import'),
    ...
    {
        $this->actingAsSuperAdmin();
        $europa = Tenant::factory()->named('Europa')->create(['is_active' => false]);
    ...
    public function test_import_dry_run_does_not_persist_changes(): void
    {
        ...
        $this->assertSame(0, Tenant::count(), 'Dry-run must not persist any rows.');
    }
