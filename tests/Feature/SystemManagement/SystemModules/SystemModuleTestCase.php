...
/**
 * Base test case para SystemModules. El Observer auto-crea permissions cuando
 * se crea un módulo.
 */
abstract class SystemModuleTestCase extends TestCase
{
    use RefreshDatabase;
    ...
    protected function seedRolesAndPermissions(): void
    {
        ...
        foreach (['view', 'show', 'create', 'edit', 'delete', 'export'] as $action) {
            Permission::firstOrCreate(['name' => "system_modules.{$action}", 'guard_name' => 'web']);
        }
    ...
    /** Payload válido. permission_key se auto-deriva del name. */
    protected function validSystemModuleData(array $overrides = []): array
    {
        return array_merge([
            'name'      => 'TestModule' . uniqid(),
            'is_active' => true,
        ], $overrides);
    }
}
