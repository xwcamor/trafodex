...
/**
 * Locks down the 3-layer duplicate prevention for SystemModules:
 *
 *   1. Form submission (StoreRequest / UpdateRequest via UniqueNormalizedName Rule)
 *   2. Excel/CSV import (SystemModulesImport: in-file dedup + DB lookup)
 *   3. DB-level partial unique index (final safety net)
 *
 * If you clone SystemModules to build a new module, copy this file and rename.
 */
class SystemModuleDuplicatePreventionTest extends SystemModuleTestCase
{
    // ─── FORM-LEVEL ─────────────────────────────────────────────────────────
    ...
    public function test_create_blocks_exact_name_duplicate(): void
    {
        ...
        SystemModule::factory()->named('América del Sur')->create();
        ...
        $this->assertSame(1, SystemModule::count());
    }
    ...
    public function test_create_blocks_case_insensitive_duplicate(): void
    {
        ...
        $this->actingAsSuperAdmin();
        SystemModule::factory()->named('Europa')->create();
    ...
    public function test_create_allows_reuse_of_soft_deleted_name(): void
    {
        ...
        $this->actingAsSuperAdmin();
        SystemModule::factory()->named('Antártida')->trashed()->create();
        ...
        $this->assertSame(1, SystemModule::count());
        $this->assertSame(2, SystemModule::withTrashed()->count());
    }

    public function test_update_can_save_same_name_for_same_record(): void
    {
        $this->actingAsSuperAdmin();
        $system_module = SystemModule::factory()->named('Europa')->create();

        $response = $this->put(route('system_management.system_modules.update', $system_module->slug), $this->validSystemModuleData(['name' => 'Europa', 'is_active' => true]));
    ...
    public function test_update_blocks_duplicate_against_other_record(): void
    {
        $this->actingAsSuperAdmin();
        SystemModule::factory()->named('Europa')->create();
        $other = SystemModule::factory()->named('Asia')->create();

        $response = $this->put(route('system_management.system_modules.update', $other->slug), $this->validSystemModuleData(['name' => 'Europa', 'is_active' => true]));
    ...
    public function test_import_dry_run_flags_in_file_duplicates(): void
    {
        ...
        $response = $this->post(
            route('system_management.system_modules.import'),
    ...
    public function test_import_commit_skips_existing_in_create_only_mode(): void
    {
        ...
        $this->assertSame(2, SystemModule::count());
    }

    public function test_import_commit_updates_existing_in_update_or_create_mode(): void
    {
        $this->actingAsSuperAdmin();
        $europa = SystemModule::factory()->named('Europa')->create(['is_active' => false]);
    ...
    public function test_import_dry_run_does_not_persist_changes(): void
    {
        ...
        $this->assertSame(0, SystemModule::count(), 'Dry-run must not persist any rows.');
    }
    ...
    protected function csv(array $rows): UploadedFile
    {
        ...
        return new UploadedFile($tmp, 'system_modules.csv', 'text/csv', null, true);
    }
}
