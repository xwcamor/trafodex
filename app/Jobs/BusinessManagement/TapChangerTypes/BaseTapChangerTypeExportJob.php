 ...
 *
 * Clon de BaseRegionExportJob adaptado a TapChangerTypes (per-tenant, con relacion
 ...
 */
abstract class BaseTapChangerTypeExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('tap_changer_types.export_job_memory_limit', '512M'));
     ...
     *
     * TapChangerTypes es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        $base = TapChangerType::query()->withoutGlobalScope('tenant');

        // tap_changer_types es catálogo global (sin tenant), no filtramos por tenant_id.
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('tap_changer_types.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('tap_changer_types.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('tap_changer_types.code'), 'value' => (string) $f['code']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('tap_changer_types.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('tap_changer_types.export_filename'));
