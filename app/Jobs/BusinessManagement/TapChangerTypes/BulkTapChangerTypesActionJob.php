 ...
 * dispatch viven en TapChangerTypeService.
 *
 ...
 */
class BulkTapChangerTypesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:tapChangerTypes:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/tap_changer_types.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('tap_changer_types.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(TapChangerTypeService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkTapChangerTypesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $tapChangerTypes = match ($this->action) {
                'restore' => TapChangerType::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => TapChangerType::whereIn('id', $chunk)->get(),
            };

            foreach ($tapChangerTypes as $tapChangerType) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($tapChangerType, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $tapChangerType),
                        'restore'    => $service->restore($tapChangerType),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkTapChangerTypesActionJob: error on tapChangerType {$tapChangerType->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkTapChangerTypesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkTapChangerTypesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkTapChangerTypesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(TapChangerTypeService $service, TapChangerType $tapChangerType): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $tapChangerType->is_active === $target) return;
        $service->update($tapChangerType, ['is_active' => $target]);
    }
}
