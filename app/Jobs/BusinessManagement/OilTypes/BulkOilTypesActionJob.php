 ...
 * dispatch viven en OilTypeService.
 *
 ...
 */
class BulkOilTypesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:oilTypes:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/oil_types.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('oil_types.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(OilTypeService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkOilTypesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $oilTypes = match ($this->action) {
                'restore' => OilType::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => OilType::whereIn('id', $chunk)->get(),
            };

            foreach ($oilTypes as $oilType) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($oilType, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $oilType),
                        'restore'    => $service->restore($oilType),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkOilTypesActionJob: error on oilType {$oilType->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkOilTypesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkOilTypesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkOilTypesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(OilTypeService $service, OilType $oilType): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $oilType->is_active === $target) return;
        $service->update($oilType, ['is_active' => $target]);
    }
}
