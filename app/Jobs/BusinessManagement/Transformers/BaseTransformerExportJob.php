 ...
 *
 * Clon de BaseRegionExportJob adaptado a Transformers (per-tenant, con relacion
 ...
 */
abstract class BaseTransformerExportJob implements ShouldQueue
{
    ...
    /**
     * Clientes a los que el user está RESTRINGIDO (3ª capa). Capturado al
     * construir (web request) porque el worker no tiene sesión y el global
     * scope assigned_customers no aplica allí. Vacío = sin restricción.
     *
     * @var int[]
     */
     ...
     *
     * Transformers es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        $columns = $this->options['columns']
            ?? array_keys(\App\Support\Transformers\TransformerExportColumns::definitions($this->userTimezone));

        $base = Transformer::query()->withoutGlobalScope('tenant');
        ...
        if ($this->tenantId !== null) {
            $base->where('transformers.tenant_id', $this->tenantId);
        }

        // 3ª capa: usuario restringido a clientes exporta SOLO esos clientes
        // (espeja el global scope assigned_customers, que no corre en el worker).
        if (!empty($this->allowedCustomerIds)) {
            $base->whereIn('transformers.customer_id', $this->allowedCustomerIds);
        }

        // Eager-load solo las relaciones de las columnas pedidas (cliente, aceite,
        // tipo, etc.) — evita N+1 al resolver $c->customer?->name y similares.
        $relations = \App\Support\Transformers\TransformerExportColumns::relationsFor($columns, $this->userTimezone);
        if ($relations) {
            $base->with($relations);
        }

        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('transformers.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['serial'])) {
            $serials = is_array($f['serial']) ? $f['serial'] : [$f['serial']];
            $out[] = ['label' => __('transformers.serial'), 'value' => implode(', ', $serials)];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('transformers.export_filename'));
