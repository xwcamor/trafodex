 ...
 * dispatch viven en LaboratoryService.
 *
 ...
 */
class BulkLaboratoriesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:laboratories:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/laboratories.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('laboratories.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(LaboratoryService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkLaboratoriesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $laboratories = match ($this->action) {
                'restore' => Laboratory::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Laboratory::whereIn('id', $chunk)->get(),
            };

            foreach ($laboratories as $laboratory) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($laboratory, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $laboratory),
                        'restore'    => $service->restore($laboratory),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkLaboratoriesActionJob: error on laboratory {$laboratory->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkLaboratoriesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkLaboratoriesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkLaboratoriesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(LaboratoryService $service, Laboratory $laboratory): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $laboratory->is_active === $target) return;
        $service->update($laboratory, ['is_active' => $target]);
    }
}
