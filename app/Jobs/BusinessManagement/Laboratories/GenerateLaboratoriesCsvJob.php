 ...
 */
class GenerateLaboratoriesCsvJob extends BaseLaboratoryExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'laboratories_csv') . '.csv';
        ...
        try {
            ...
            $headings = [
                'id'         => __('laboratories.id'),
                'name'       => __('laboratories.name'),
                'code'       => __('laboratories.code'),
                'sort_order' => __('laboratories.sort_order'),
                'is_active'  => __('laboratories.is_active'),
                'slug'       => 'Slug',
            ...
            $this->buildQuery()->chunkById(1000, function ($laboratories) use ($handle, $columns, $tz) {
                foreach ($laboratories as $laboratory) {
                    $row = array_map(fn ($col) => match ($col) {
                        'id'         => $laboratory->id,
                        'name'       => $laboratory->name,
                        'code'       => $laboratory->code ?? '',
                        'sort_order' => $laboratory->sort_order ?? '',
                        'is_active'  => $laboratory->is_active ? '1' : '0',
                        'slug'       => $laboratory->slug,
                        'created_at' => $laboratory->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'updated_at' => $laboratory->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'creator'    => $laboratory->creator?->name ?? '',
                        default      => $laboratory->{$col} ?? '',
                    }, $columns);
                    fputcsv($handle, $row);
                }
            }, 'laboratories.id', 'id');
