 ...
 */
class GenerateSettingsCsvJob extends BaseSettingExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'settings_csv') . '.csv';
        ...
        try {
            ...
            $headings = [
                ...
                'is_active'  => __('settings.is_active'),
                'slug'       => 'Slug',
            ...
            $this->buildQuery()->chunkById(1000, function ($settings) use ($handle, $columns, $tz) {
                foreach ($settings as $setting) {
                    $row = array_map(fn ($col) => match ($col) {
                        ...
                        'slug'       => $setting->slug,
                        'created_at' => $setting->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'updated_at' => $setting->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'creator'    => $setting->creator?->name ?? '',
                        default      => $setting->{$col} ?? '',
                    }, $columns);
                    fputcsv($handle, $row);
                }
