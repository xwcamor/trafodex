 ...
 */
abstract class BaseTenantExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('tenants.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        // El plan se deriva de la suscripción vigente. Eager-load siempre — es
        // un hasOne barato y evita N+1 en el chunk loop del export.
        $base->with('activeSubscription');
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('tenants.name'),  'value' => implode(', ', $names)];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('tenants.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('tenants.export_filename'));
