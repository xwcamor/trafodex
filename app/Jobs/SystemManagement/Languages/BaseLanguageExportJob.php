 ...
 */
abstract class BaseLanguageExportJob implements ShouldQueue
{
    ...
    /** TZ del user que disparó el export — las fechas se convierten a este TZ
     *  al display. BD sigue en UTC. */
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('languages.export_job_memory_limit', '512M'));
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('languages.name'),  'value' => implode(', ', $names)];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('languages.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('languages.export_filename'));
