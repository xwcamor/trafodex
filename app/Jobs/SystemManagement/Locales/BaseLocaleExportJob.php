 ...
 */
abstract class BaseLocaleExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('locales.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        if (in_array('language', $cols, true)) {
            $base->with('language:id,name,iso_code');
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('locales.name'),  'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $codes = is_array($f['code']) ? $f['code'] : [$f['code']];
            $out[] = ['label' => __('locales.code'), 'value' => implode(', ', $codes)];
        }
        if (!empty($f['language_id'])) {
            $ids = is_array($f['language_id']) ? $f['language_id'] : [$f['language_id']];
            $names = \App\Models\Language::whereIn('id', $ids)->pluck('name')->all();
            $out[] = ['label' => __('locales.language'), 'value' => implode(', ', $names)];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('locales.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('locales.export_filename'));
