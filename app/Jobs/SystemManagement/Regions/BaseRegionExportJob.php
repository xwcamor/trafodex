 ...
 */
abstract class BaseRegionExportJob implements ShouldQueue
{
    ...
    /** TZ del user que disparó el export — todas las fechas del archivo se
     *  convierten a este TZ al display. La BD sigue en UTC. */
    ...
    public function __construct(int $userId, array $options = [])
    {
        ...
        // Resolvemos el TZ aquí (en proceso síncrono del dispatcher) — el
        // worker que ejecute el job no tiene `auth()`, así que serializamos
        // el resultado ya resuelto. Tz::for() devuelve el TZ del user, o
        // del tenant si user no tiene override, o del country, o UTC.
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('regions.export_job_memory_limit', '512M'));
    ...
    protected function buildQuery()
    {
        ...
        if (in_array('creator', $this->options['columns'] ?? ['creator'])) {
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('regions.name'),  'value' => implode(', ', $names)];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('regions.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('regions.export_filename'));
