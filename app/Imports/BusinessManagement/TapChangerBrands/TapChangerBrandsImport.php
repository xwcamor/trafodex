...
namespace App\Imports\BusinessManagement\TapChangerBrands;
...
/**
 * Imports TapChangerBrands from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * tap_changer_brands es un CATALOGO GLOBAL (sin tenant): el import NO scope-a por
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos tapChangerBrands hay HOY +
 ...
 */
class TapChangerBrandsImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count global de tapChangerBrands (pre-import). Catalogo unico, sin tenant. */
    ...
    public function __construct(
        ...
        // Snapshot del count actual. tap_changer_brands es catalogo GLOBAL.
        $this->currentCount = TapChangerBrand::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo tapChangerBrand
            ...
            foreach ($rows as $i => $row) {
                ...
                if ($existing) {
                    ...
                    TapChangerBrand::create([
                        'name'        => $name,
                        'code'        => $code,
                        'is_active'   => true,
                        'created_by'  => Auth::id(),
                        // Sin tenant_id: tap_changer_brands es catalogo global. El slug
    ...
    {
        return TapChangerBrand::query()->withoutGlobalScopes()
     ...
     * Global: tap_changer_brands es catalogo unico, sin scope por tenant.
     */
    protected function findExistingByNameInsensitive(string $name): ?TapChangerBrand
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = TapChangerBrand::query()->withoutGlobalScopes();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(tap_changer_brands.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(tap_changer_brands.name) = LOWER(?)', [$name]);
        }
