...
namespace App\Imports\BusinessManagement\TapChangerTechnologies;
...
/**
 * Imports TapChangerTechnologies from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * tap_changer_technologies es un CATALOGO GLOBAL (sin tenant): el import NO scope-a por
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos tapChangerTechnologies hay HOY +
 ...
 */
class TapChangerTechnologiesImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count global de tapChangerTechnologies (pre-import). Catalogo unico, sin tenant. */
    ...
    public function __construct(
        ...
        // Snapshot del count actual. tap_changer_technologies es catalogo GLOBAL.
        $this->currentCount = TapChangerTechnology::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo tapChangerTechnology
            ...
            foreach ($rows as $i => $row) {
                ...
                if ($existing) {
                    ...
                    TapChangerTechnology::create([
                        'name'        => $name,
                        'code'        => $code,
                        'is_active'   => true,
                        'created_by'  => Auth::id(),
                        // Sin tenant_id: tap_changer_technologies es catalogo global. El slug
    ...
    {
        return TapChangerTechnology::query()->withoutGlobalScopes()
     ...
     * Global: tap_changer_technologies es catalogo unico, sin scope por tenant.
     */
    protected function findExistingByNameInsensitive(string $name): ?TapChangerTechnology
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = TapChangerTechnology::query()->withoutGlobalScopes();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(tap_changer_technologies.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(tap_changer_technologies.name) = LOWER(?)', [$name]);
        }
