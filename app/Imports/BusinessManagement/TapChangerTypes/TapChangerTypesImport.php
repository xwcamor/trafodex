...
namespace App\Imports\BusinessManagement\TapChangerTypes;
...
/**
 * Imports TapChangerTypes from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * tap_changer_types es un CATALOGO GLOBAL (sin tenant): el import NO scope-a por
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos tapChangerTypes hay HOY +
 ...
 */
class TapChangerTypesImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count global de tapChangerTypes (pre-import). Catalogo unico, sin tenant. */
    ...
    public function __construct(
        ...
        // Snapshot del count actual. tap_changer_types es catalogo GLOBAL.
        $this->currentCount = TapChangerType::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo tapChangerType
            ...
            foreach ($rows as $i => $row) {
                ...
                if ($existing) {
                    ...
                    TapChangerType::create([
                        'name'        => $name,
                        'code'        => $code,
                        'is_active'   => true,
                        'created_by'  => Auth::id(),
                        // Sin tenant_id: tap_changer_types es catalogo global. El slug
    ...
    {
        return TapChangerType::query()->withoutGlobalScopes()
     ...
     * Global: tap_changer_types es catalogo unico, sin scope por tenant.
     */
    protected function findExistingByNameInsensitive(string $name): ?TapChangerType
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = TapChangerType::query()->withoutGlobalScopes();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(tap_changer_types.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(tap_changer_types.name) = LOWER(?)', [$name]);
        }
