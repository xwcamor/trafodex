...
namespace App\Imports\BusinessManagement\Laboratories;
...
/**
 * Imports Laboratories from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * laboratories es un CATALOGO GLOBAL (sin tenant): el import NO scope-a por
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos laboratories hay HOY +
 ...
 */
class LaboratoriesImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count global de laboratories (pre-import). Catalogo unico, sin tenant. */
    ...
    public function __construct(
        ...
        // Snapshot del count actual. laboratories es catalogo GLOBAL.
        $this->currentCount = Laboratory::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo laboratory
            ...
            foreach ($rows as $i => $row) {
                ...
                if ($existing) {
                    ...
                    Laboratory::create([
                        ...
                        'is_active'   => true,
                        'created_by'  => Auth::id(),
                        // Sin tenant_id: laboratories es catalogo global. El slug
    ...
    {
        return Laboratory::query()->withoutGlobalScopes()
     ...
     * Global: laboratories es catalogo unico, sin scope por tenant.
     */
    protected function findExistingByNameInsensitive(string $name): ?Laboratory
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Laboratory::query()->withoutGlobalScopes();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(laboratories.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(laboratories.name) = LOWER(?)', [$name]);
        }
