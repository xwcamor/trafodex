...
namespace App\Imports\BusinessManagement\OilTypes;
...
/**
 * Imports OilTypes from .xlsx/.csv.
 *
 * Columns:
 *   - name  (required, max 255, unico global case/accent-insensitive)
 *   - code  (optional, max 40, identificador tecnico unico global)
 *
 * El import NO maneja is_active: toda alta nace activa (coherente con clientes).
 * El estado se gestiona desde la UI / bulk actions, no por archivo.
 *
 * Modes: 'create_only' | 'update_or_create'
 *
 * oil_types es un CATALOGO GLOBAL (sin tenant): el import NO scope-a por
 * tenant_id (la tabla no tiene esa columna; filtrarla revienta en Postgres).
 *
 * 3-layer duplicate protection (global, por name y por code):
 ...
 *   3. DB: unique constraint de `slug` (auto-generado en el modelo) + name unico
 *
 * Enforce `Tenant::maxRecordsPerModule()`:
 *   - Si el plan del usuario tiene limite, contamos cuantos oilTypes hay HOY +
 ...
 */
class OilTypesImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count global de oilTypes (pre-import). Catalogo unico, sin tenant. */
    ...
    public function __construct(
        ...
        // Snapshot del count actual. oil_types es catalogo GLOBAL.
        $this->currentCount = OilType::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo oilType
            ...
            foreach ($rows as $i => $row) {
                ...
                if ($existing) {
                    ...
                    // Las altas nacen activas. El import no importa registros
                    // inactivos (coherente con el módulo de clientes): el estado
                    // se gestiona después desde la UI / bulk actions.
                    OilType::create([
                        'name'        => $name,
                        'code'        => $code,
                        'is_active'   => true,
                        'created_by'  => Auth::id(),
                        // Sin tenant_id: oil_types es catalogo global. El slug
    ...
    /** Trim → null si vacío. El code es el identificador técnico (mineral, etc). */
    ...
    {
        return OilType::query()->withoutGlobalScopes()
     ...
     * Global: oil_types es catalogo unico, sin scope por tenant.
     */
    protected function findExistingByNameInsensitive(string $name): ?OilType
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = OilType::query()->withoutGlobalScopes();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(oil_types.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(oil_types.name) = LOWER(?)', [$name]);
        }
