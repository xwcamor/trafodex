...
namespace App\Imports\BusinessManagement\TapChangerModels;
...
/**
 * Imports TapChangerModels from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * tap_changer_models es un CATALOGO GLOBAL (sin tenant): el import NO scope-a por
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos tapChangerModels hay HOY +
 ...
 */
class TapChangerModelsImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count global de tapChangerModels (pre-import). Catalogo unico, sin tenant. */
    ...
    public function __construct(
        ...
        // Snapshot del count actual. tap_changer_models es catalogo GLOBAL.
        $this->currentCount = TapChangerModel::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo tapChangerModel
            ...
            foreach ($rows as $i => $row) {
                ...
                if ($existing) {
                    ...
                    TapChangerModel::create([
                        'name'        => $name,
                        'code'        => $code,
                        'is_active'   => true,
                        'created_by'  => Auth::id(),
                        // Sin tenant_id: tap_changer_models es catalogo global. El slug
    ...
    {
        return TapChangerModel::query()->withoutGlobalScopes()
     ...
     * Global: tap_changer_models es catalogo unico, sin scope por tenant.
     */
    protected function findExistingByNameInsensitive(string $name): ?TapChangerModel
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = TapChangerModel::query()->withoutGlobalScopes();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(tap_changer_models.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(tap_changer_models.name) = LOWER(?)', [$name]);
        }
