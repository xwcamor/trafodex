...
/**
 * Generates a styled .docx report of Roles using PhpWord directly
...
class RolesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($role) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'                => ['heading' => 'N°',                          'value' => fn($r) => (string) $r->id],
            'name'              => ['heading' => __('roles.name'),              'value' => fn($r) => (string) $r->name],
            'description'       => ['heading' => __('roles.description'),       'value' => fn($r) => (string) ($r->description ?? '')],
            'is_active'         => ['heading' => __('roles.is_active'),         'value' => fn($r) => $r->state_text],
            'permissions_count' => ['heading' => __('roles.permissions_count'), 'value' => fn($r) => (string) ($r->permissions_count ?? $r->permissions()->count())],
            'users_count'       => ['heading' => __('roles.users_count'),       'value' => fn($r) => (string) ($r->users_count ?? $r->users()->count())],
            'tenant'            => ['heading' => __('roles.tenant'),            'value' => fn($r) => (string) ($r->tenant?->name ?? '—')],
        ...
        $title         = $options['title']         ?? __('roles.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('RolesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('RolesTable');
            ...
            foreach ($roles as $i => $role) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($role);

                    if ($col === 'is_active') {
                        $color = $role->is_active ? '1D7044' : 'C8281D';
