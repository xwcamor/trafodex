 ...
 *
 * Clon de CustomersExport con columnas de Roles: id, name, description,
 * is_active, permissions_count, users_count, tenant, slug, created_at, creator.
 */
class RolesExport implements FromCollection, WithEvents, WithTitle
{
    protected $roles;
    protected int $count;
    ...
    public function __construct($roles, array $options = [], ?int $count = null)
    {
        ...
        $this->roles   = is_array($roles) ? collect($roles) : $roles;
        $this->options = $options;
        $this->count   = $count !== null
            ? $count
            : (is_countable($roles) ? count($roles) : iterator_count($roles));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'                => ['heading' => 'N°',                              'value' => fn($r, $i) => $r->id],
            'name'              => ['heading' => __('roles.name'),                  'value' => fn($r, $i) => $r->name],
            'description'       => ['heading' => __('roles.description'),           'value' => fn($r, $i) => $r->description ?? ''],
            'is_active'         => ['heading' => __('roles.is_active'),             'value' => fn($r, $i) => $r->state_text],
            'permissions_count' => ['heading' => __('roles.permissions_count'),     'value' => fn($r, $i) => $r->permissions_count ?? $r->permissions()->count()],
            'users_count'       => ['heading' => __('roles.users_count'),           'value' => fn($r, $i) => $r->users_count ?? $r->users()->count()],
            'tenant'            => ['heading' => __('roles.tenant'),                'value' => fn($r, $i) => $r->tenant?->name ?? '—'],
    ...
    public function collection()
    {
        ...
        foreach ($this->roles as $role) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($role, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('roles.export_title'), 0, 31);
    }
