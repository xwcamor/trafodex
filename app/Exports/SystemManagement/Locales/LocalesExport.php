 ...
 */
class LocalesExport implements FromCollection, WithEvents, WithTitle
{
    protected $locales;
    protected int $count;
    ...
    public function __construct($locales, array $options = [], ?int $count = null)
    {
        ...
        $this->locales = is_array($locales) ? collect($locales) : $locales;
        $this->options = $options;
        $this->count   = $count !== null
            ? $count
            : (is_countable($locales) ? count($locales) : iterator_count($locales));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('locales.id'),       'value' => fn($r, $i) => $r->id],
            'name'       => ['heading' => __('locales.name'),     'value' => fn($r, $i) => $r->name],
            'code'       => ['heading' => __('locales.code'),     'value' => fn($r, $i) => $r->code],
            'language'   => ['heading' => __('locales.language'), 'value' => fn($r, $i) => $r->language?->name ?? '—'],
            'is_active'  => ['heading' => __('locales.is_active'),'value' => fn($r, $i) => $r->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('locales.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        foreach ($this->locales as $locale) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($locale, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('locales.export_title'), 0, 31);
    }
