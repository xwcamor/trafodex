...
/**
 * Generates a styled .docx report of Locales using PhpWord directly
...
class LocalesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($locale) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('locales.id'),        'value' => fn($r) => (string) $r->id],
            'name'       => ['heading' => __('locales.name'),      'value' => fn($r) => (string) $r->name],
            'code'       => ['heading' => __('locales.code'),      'value' => fn($r) => (string) $r->code],
            'language'   => ['heading' => __('locales.language'),  'value' => fn($r) => $r->language?->name ?? '—'],
            'is_active'  => ['heading' => __('locales.is_active'), 'value' => fn($r) => $r->state_text],
        ...
        $title         = $options['title']         ?? __('locales.export_title');
        ...
        if ($count === 0) {
            ...
            // Table style
            $phpWord->addTableStyle('LocalesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('LocalesTable');
