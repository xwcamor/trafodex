...
/**
 * Generates a styled .docx report of Tenants using PhpWord directly
...
class TenantsWord
{
    /** Map column key → ['heading' => string, 'value' => fn($tenant) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($tenants) ? count($tenants) : iterator_count($tenants));

        $this->columnDefs = [
            'id'         => ['heading' => __('tenants.id'),        'value' => fn($r) => (string) $r->id],
            'name'       => ['heading' => __('tenants.name'),      'value' => fn($r) => (string) $r->name],
            'plan'       => ['heading' => __('tenants.plan'),      'value' => fn($r) => strtoupper($r->currentPlan())],
            'is_active'  => ['heading' => __('tenants.is_active'), 'value' => fn($r) => $r->state_text],
        ...
        $title         = $options['title']         ?? __('tenants.export_title');
        ...
        if ($count === 0) {
            ...
            // Table style
            $phpWord->addTableStyle('TenantsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('TenantsTable');
