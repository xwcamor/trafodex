 ...
 */
class TenantsExport implements FromCollection, WithEvents, WithTitle
{
    protected $tenants;
    protected int $count;
    ...
    public function __construct($tenants, array $options = [], ?int $count = null)
    {
        ...
        $this->tenants = is_array($tenants) ? collect($tenants) : $tenants;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('tenants.id'),       'value' => fn($r, $i) => $r->id],
            'name'       => ['heading' => __('tenants.name'),     'value' => fn($r, $i) => $r->name],
            'plan'       => ['heading' => __('tenants.plan'),     'value' => fn($r, $i) => strtoupper($r->currentPlan())],
            'is_active'  => ['heading' => __('tenants.is_active'),'value' => fn($r, $i) => $r->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->tenants as $tenant) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($tenant, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('tenants.export_title'), 0, 31);
    }
