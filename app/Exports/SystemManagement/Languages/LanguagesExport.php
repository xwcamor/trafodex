 ...
 */
class LanguagesExport implements FromCollection, WithEvents, WithTitle
{
    protected $languages;
    protected int $count;
    ...
    /** TZ del user que disparó el export — todas las fechas se convierten al display. */
    protected string $tz;

    public function __construct($languages, array $options = [], ?int $count = null)
    {
        ...
        $this->languages = is_array($languages) ? collect($languages) : $languages;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($languages) ? count($languages) : iterator_count($languages));

        // Priorizamos 'timezone' inyectado por el Job (ya resuelto). Si no
        // viene, lo derivamos del user_id. Si tampoco, UTC.
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('languages.id'),       'value' => fn($r, $i) => $r->id],
            'name'       => ['heading' => __('languages.name'),     'value' => fn($r, $i) => $r->name],
            'iso_code'   => ['heading' => __('languages.iso_code'), 'value' => fn($r, $i) => $r->iso_code],
            'is_active'  => ['heading' => __('languages.is_active'),'value' => fn($r, $i) => $r->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->languages as $language) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($language, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('languages.export_title'), 0, 31);
    }
