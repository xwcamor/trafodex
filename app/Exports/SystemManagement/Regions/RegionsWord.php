...
class RegionsWord
{
    /** Map column key → ['heading' => string, 'value' => fn($region) => mixed] */
    ...
    public function generate(
        ...
        // TZ del user que disparó el export, ya resuelto upstream.
        ...
        $count = $count !== null
            ? $count
            : (is_countable($regions) ? count($regions) : iterator_count($regions));

        $this->columnDefs = [
            'id'         => ['heading' => __('regions.id'),        'value' => fn($r) => (string) $r->id],
            'name'       => ['heading' => __('regions.name'),      'value' => fn($r) => (string) $r->name],
            'is_active'  => ['heading' => __('regions.is_active'), 'value' => fn($r) => $r->state_text],
        ...
        $title         = $options['title']         ?? __('regions.export_title');
