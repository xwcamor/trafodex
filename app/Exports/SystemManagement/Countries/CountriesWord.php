...
/**
 * Generates a styled .docx report of Countries using PhpWord directly
...
class CountriesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($country) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($countries) ? count($countries) : iterator_count($countries));

        $this->columnDefs = [
            'id'             => ['heading' => __('countries.id'),               'value' => fn($r) => (string) $r->id],
            'name'           => ['heading' => __('countries.name'),             'value' => fn($r) => (string) $r->name],
            'iso_code'       => ['heading' => __('countries.iso_code'),         'value' => fn($r) => (string) $r->iso_code],
            'currency'       => ['heading' => __('countries.currency'),         'value' => fn($r) => (string) $r->currency],
            'timezone'       => ['heading' => __('countries.timezone'),         'value' => fn($r) => (string) $r->timezone],
            'region'         => ['heading' => __('countries.region'),           'value' => fn($r) => $r->region?->name ?? '—'],
            'default_locale' => ['heading' => __('countries.default_locale'),   'value' => fn($r) => $r->defaultLocale?->code ?? '—'],
            'is_active'      => ['heading' => __('countries.is_active'),        'value' => fn($r) => $r->state_text],
        ...
        $title         = $options['title']         ?? __('countries.export_title');
        ...
        if ($count === 0) {
            ...
            // Table style
            $phpWord->addTableStyle('CountriesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('CountriesTable');
