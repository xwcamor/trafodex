 ...
 */
class CountriesExport implements FromCollection, WithEvents, WithTitle
{
    protected $countries;
    protected int $count;
    ...
    public function __construct($countries, array $options = [], ?int $count = null)
    {
        ...
        $this->countries = is_array($countries) ? collect($countries) : $countries;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'             => ['heading' => __('countries.id'),               'value' => fn($r, $i) => $r->id],
            'name'           => ['heading' => __('countries.name'),             'value' => fn($r, $i) => $r->name],
            'iso_code'       => ['heading' => __('countries.iso_code'),         'value' => fn($r, $i) => $r->iso_code],
            'currency'       => ['heading' => __('countries.currency'),         'value' => fn($r, $i) => $r->currency],
            'timezone'       => ['heading' => __('countries.timezone'),         'value' => fn($r, $i) => $r->timezone],
            'region'         => ['heading' => __('countries.region'),           'value' => fn($r, $i) => $r->region?->name ?? '—'],
            'default_locale' => ['heading' => __('countries.default_locale'),   'value' => fn($r, $i) => $r->defaultLocale?->code ?? '—'],
            'is_active'      => ['heading' => __('countries.is_active'),        'value' => fn($r, $i) => $r->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->countries as $country) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($country, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('countries.export_title'), 0, 31);
    }
