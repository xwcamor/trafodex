 ...
 */
class SystemModulesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($system_modules, array $options = [], ?int $count = null)
    {
        ...
        $this->system_modules = is_array($system_modules) ? collect($system_modules) : $system_modules;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('system_modules.id'),       'value' => fn($r, $i) => $r->id],
            'name'       => ['heading' => __('system_modules.name'),     'value' => fn($r, $i) => $r->name],
            'is_active'  => ['heading' => __('system_modules.is_active'),'value' => fn($r, $i) => $r->state_text],
    ...
    public function collection()
    {
        ...
        // Row 5+ — data
        $i = 0;
        foreach ($this->system_modules as $system_module) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($system_module, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('system_modules.export_title'), 0, 31);
    }
