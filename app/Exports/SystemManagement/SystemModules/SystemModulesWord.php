...
/**
 * Generates a styled .docx report of SystemModules using PhpWord directly
 ...
 */
class SystemModulesWord
{
    /** Map column key → ['heading' => string, 'value' => fn($system_module) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($system_modules) ? count($system_modules) : iterator_count($system_modules));

        $this->columnDefs = [
            'id'         => ['heading' => __('system_modules.id'),        'value' => fn($r) => (string) $r->id],
            'name'       => ['heading' => __('system_modules.name'),      'value' => fn($r) => (string) $r->name],
            'is_active'  => ['heading' => __('system_modules.is_active'), 'value' => fn($r) => $r->state_text],
        ...
        $title         = $options['title']         ?? __('system_modules.export_title');
        ...
        if ($count === 0) {
            ...
            // Table style
            $phpWord->addTableStyle('SystemModulesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('SystemModulesTable');
            ...
            // Data rows (zebra)
            foreach ($system_modules as $i => $system_module) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
