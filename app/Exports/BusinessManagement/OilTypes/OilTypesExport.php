...
namespace App\Exports\BusinessManagement\OilTypes;
 ...
 */
class OilTypesExport implements FromCollection, WithEvents, WithTitle
{
    protected $oilTypes;
    protected int $count;
    ...
    public function __construct($oilTypes, array $options = [], ?int $count = null)
    {
        ...
        $this->oilTypes = is_array($oilTypes) ? collect($oilTypes) : $oilTypes;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($oilTypes) ? count($oilTypes) : iterator_count($oilTypes));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('oil_types.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('oil_types.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('oil_types.code'),       'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('oil_types.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('oil_types.is_active'),  'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('oil_types.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        // Row 5+ â€” data
        $i = 0;
        foreach ($this->oilTypes as $oilType) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($oilType, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('oil_types.export_title'), 0, 31);
    }
