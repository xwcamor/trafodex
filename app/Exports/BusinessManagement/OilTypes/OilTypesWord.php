...
/**
 * Generates a styled .docx report of OilTypes using PhpWord directly
...
class OilTypesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($oilType) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('oil_types.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('oil_types.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('oil_types.code'),       'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('oil_types.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('oil_types.is_active'),  'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('oil_types.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('OilTypesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('OilTypesTable');
            ...
            foreach ($oilTypes as $i => $oilType) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($oilType);

                    if ($col === 'is_active') {
                        $color = $oilType->is_active ? '1D7044' : 'C8281D';
