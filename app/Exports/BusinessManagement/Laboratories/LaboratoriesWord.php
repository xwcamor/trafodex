...
/**
 * Generates a styled .docx report of Laboratories using PhpWord directly
 ...
 */
class LaboratoriesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($laboratory) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('laboratories.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('laboratories.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('laboratories.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('laboratories.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('laboratories.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('laboratories.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('LaboratoriesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('LaboratoriesTable');
            ...
            foreach ($laboratories as $i => $laboratory) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($laboratory);

                    if ($col === 'is_active') {
                        $color = $laboratory->is_active ? '1D7044' : 'C8281D';
