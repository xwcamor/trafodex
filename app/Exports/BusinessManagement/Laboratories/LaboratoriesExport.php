...
namespace App\Exports\BusinessManagement\Laboratories;
 ...
 */
class LaboratoriesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($laboratories, array $options = [], ?int $count = null)
    {
        ...
        $this->laboratories = is_array($laboratories) ? collect($laboratories) : $laboratories;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($laboratories) ? count($laboratories) : iterator_count($laboratories));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('laboratories.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('laboratories.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('laboratories.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('laboratories.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('laboratories.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('laboratories.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        foreach ($this->laboratories as $laboratory) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($laboratory, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('laboratories.export_title'), 0, 31);
    }
