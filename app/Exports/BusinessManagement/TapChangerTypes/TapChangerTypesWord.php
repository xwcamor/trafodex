...
/**
 * Generates a styled .docx report of TapChangerTypes using PhpWord directly
 ...
 */
class TapChangerTypesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($tapChangerType) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('tap_changer_types.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('tap_changer_types.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('tap_changer_types.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('tap_changer_types.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('tap_changer_types.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('tap_changer_types.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('TapChangerTypesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('TapChangerTypesTable');
            ...
            // Data rows (zebra)
            foreach ($tapChangerTypes as $i => $tapChangerType) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($tapChangerType);

                    if ($col === 'is_active') {
                        $color = $tapChangerType->is_active ? '1D7044' : 'C8281D';
