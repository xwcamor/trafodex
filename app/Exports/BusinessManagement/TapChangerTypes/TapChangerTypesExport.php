 ...
 */
class TapChangerTypesExport implements FromCollection, WithEvents, WithTitle
{
    protected $tapChangerTypes;
    protected int $count;
    ...
    public function __construct($tapChangerTypes, array $options = [], ?int $count = null)
    {
        ...
        $this->tapChangerTypes = is_array($tapChangerTypes) ? collect($tapChangerTypes) : $tapChangerTypes;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($tapChangerTypes) ? count($tapChangerTypes) : iterator_count($tapChangerTypes));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('tap_changer_types.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('tap_changer_types.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('tap_changer_types.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('tap_changer_types.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('tap_changer_types.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('tap_changer_types.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        // Row 5+ â€” data
        $i = 0;
        foreach ($this->tapChangerTypes as $tapChangerType) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($tapChangerType, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('tap_changer_types.export_title'), 0, 31);
    }
