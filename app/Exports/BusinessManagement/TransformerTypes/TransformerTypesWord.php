...
/**
 * Generates a styled .docx report of TransformerTypes using PhpWord directly
 ...
 */
class TransformerTypesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($transformerType) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('transformer_types.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('transformer_types.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('transformer_types.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('transformer_types.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('transformer_types.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('transformer_types.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('TransformerTypesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('TransformerTypesTable');
            ...
            // Data rows (zebra)
            foreach ($transformerTypes as $i => $transformerType) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($transformerType);

                    if ($col === 'is_active') {
                        $color = $transformerType->is_active ? '1D7044' : 'C8281D';
