...
namespace App\Exports\BusinessManagement\TransformerTypes;
 ...
 */
class TransformerTypesExport implements FromCollection, WithEvents, WithTitle
{
    protected $transformerTypes;
    protected int $count;
    ...
    public function __construct($transformerTypes, array $options = [], ?int $count = null)
    {
        ...
        $this->transformerTypes = is_array($transformerTypes) ? collect($transformerTypes) : $transformerTypes;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($transformerTypes) ? count($transformerTypes) : iterator_count($transformerTypes));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('transformer_types.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('transformer_types.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('transformer_types.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('transformer_types.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('transformer_types.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('transformer_types.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        // Row 5+ â€” data
        $i = 0;
        foreach ($this->transformerTypes as $transformerType) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($transformerType, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('transformer_types.export_title'), 0, 31);
    }
