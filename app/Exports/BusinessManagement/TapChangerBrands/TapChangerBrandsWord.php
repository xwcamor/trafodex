...
namespace App\Exports\BusinessManagement\TapChangerBrands;
...
/**
 * Generates a styled .docx report of TapChangerBrands using PhpWord directly
 ...
 */
class TapChangerBrandsWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($tapChangerBrand) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($tapChangerBrands) ? count($tapChangerBrands) : iterator_count($tapChangerBrands));

        $this->columnDefs = [
            'id'         => ['heading' => __('tap_changer_brands.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('tap_changer_brands.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('tap_changer_brands.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('tap_changer_brands.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('tap_changer_brands.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('tap_changer_brands.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('TapChangerBrandsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('TapChangerBrandsTable');
            ...
            // Data rows (zebra)
            foreach ($tapChangerBrands as $i => $tapChangerBrand) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($tapChangerBrand);

                    if ($col === 'is_active') {
                        $color = $tapChangerBrand->is_active ? '1D7044' : 'C8281D';
