 ...
 */
class TapChangerBrandsExport implements FromCollection, WithEvents, WithTitle
{
    protected $tapChangerBrands;
    protected int $count;
    ...
    public function __construct($tapChangerBrands, array $options = [], ?int $count = null)
    {
        ...
        $this->tapChangerBrands = is_array($tapChangerBrands) ? collect($tapChangerBrands) : $tapChangerBrands;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('tap_changer_brands.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('tap_changer_brands.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('tap_changer_brands.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('tap_changer_brands.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('tap_changer_brands.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    public function collection()
    {
        ...
        // Row 5+ â€” data
        $i = 0;
        foreach ($this->tapChangerBrands as $tapChangerBrand) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($tapChangerBrand, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('tap_changer_brands.export_title'), 0, 31);
    }
