 ...
 */
class BrandsExport implements FromCollection, WithEvents, WithTitle
{
    protected $brands;
    protected int $count;
    ...
    public function __construct($brands, array $options = [], ?int $count = null)
    {
        ...
        $this->brands = is_array($brands) ? collect($brands) : $brands;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($brands) ? count($brands) : iterator_count($brands));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('brands.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('brands.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('brands.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('brands.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('brands.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('brands.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        foreach ($this->brands as $brand) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($brand, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('brands.export_title'), 0, 31);
    }
