...
/**
 * Generates a styled .docx report of Brands using PhpWord directly
...
class BrandsWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($brand) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('brands.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('brands.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('brands.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('brands.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('brands.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('brands.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('BrandsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('BrandsTable');
            ...
            foreach ($brands as $i => $brand) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($brand);

                    if ($col === 'is_active') {
                        $color = $brand->is_active ? '1D7044' : 'C8281D';
