...
/**
 * Generates a styled .docx report of TapChangerTechnologies using PhpWord directly
 ...
 */
class TapChangerTechnologiesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($tapChangerTechnology) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('tap_changer_technologies.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('tap_changer_technologies.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('tap_changer_technologies.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('tap_changer_technologies.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('tap_changer_technologies.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('tap_changer_technologies.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('TapChangerTechnologiesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('TapChangerTechnologiesTable');
            ...
            // Data rows (zebra)
            foreach ($tapChangerTechnologies as $i => $tapChangerTechnology) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($tapChangerTechnology);

                    if ($col === 'is_active') {
                        $color = $tapChangerTechnology->is_active ? '1D7044' : 'C8281D';
