...
namespace App\Exports\BusinessManagement\TapChangerTechnologies;
 ...
 */
class TapChangerTechnologiesExport implements FromCollection, WithEvents, WithTitle
{
    protected $tapChangerTechnologies;
    protected int $count;
    ...
    public function __construct($tapChangerTechnologies, array $options = [], ?int $count = null)
    {
        ...
        $this->tapChangerTechnologies = is_array($tapChangerTechnologies) ? collect($tapChangerTechnologies) : $tapChangerTechnologies;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($tapChangerTechnologies) ? count($tapChangerTechnologies) : iterator_count($tapChangerTechnologies));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('tap_changer_technologies.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('tap_changer_technologies.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('tap_changer_technologies.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('tap_changer_technologies.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('tap_changer_technologies.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('tap_changer_technologies.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        // Row 5+ â€” data
        $i = 0;
        foreach ($this->tapChangerTechnologies as $tapChangerTechnology) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($tapChangerTechnology, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('tap_changer_technologies.export_title'), 0, 31);
    }
