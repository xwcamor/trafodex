 ...
 */
class TransformersExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($transformers, array $options = [], ?int $count = null)
    {
        ...
        $this->transformers = is_array($transformers) ? collect($transformers) : $transformers;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($transformers) ? count($transformers) : iterator_count($transformers));
        ...
        // Columnas de dominio completas, definidas en un solo lugar (compartido
        // con CSV/Word/PDF). Los resolvers toman ($c); el ($i) extra que pasa
        // collection() abajo se ignora sin problema.
        $this->columnDefs = \App\Support\Transformers\TransformerExportColumns::definitions($this->tz);
    ...
    public function collection()
    {
        ...
        foreach ($this->transformers as $transformer) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($transformer, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('transformers.export_title'), 0, 31);
    }
