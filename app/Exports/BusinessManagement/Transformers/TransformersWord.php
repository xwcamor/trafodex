...
/**
 * Generates a styled .docx report of Transformers using PhpWord directly
 ...
 */
class TransformersWord
{
    /** Map column key → ['heading' => string, 'value' => fn($transformer) => mixed] */
    ...
    public function generate(
        ...
        // Columnas de dominio completas (definición compartida con Excel/CSV/PDF).
        $this->columnDefs = \App\Support\Transformers\TransformerExportColumns::definitions($tz);

        $title         = $options['title']         ?? __('transformers.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('TransformersTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('TransformersTable');
            ...
            foreach ($transformers as $i => $transformer) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($transformer);
