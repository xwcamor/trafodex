...
/**
 * TapChangerType — catálogo de tipos de conmutador (OLTC/DETC: en vacío,
 * bajo carga, modelos específicos…).
 *
 * Metadato descriptivo del transformador (NO es eje de diagnóstico). Catálogo
 * GLOBAL (sin tenant): todos los workspaces ven el mismo catálogo, por eso NO
 * usa BelongsToTenant. Mantiene SoftDeletes + Auditable + HasFavorites.
 */
class TapChangerType extends Model
{
    use HasFactory, SoftDeletes, Auditable, HasFavorites;

    protected string $auditModule = 'tap_changer_types';
    ...
    /** Transformadores con este tipo de conmutador (FK tap_changer_type_id). */
    ...
    /**
     * scopeFilter — mismo patrón que Customer, sobre la tabla tap_changer_types.
    ...
    public function scopeFilter($query, $request)
    {
        ...
        if (in_array($sort, ['id', 'name', 'code', 'is_active', 'sort_order', 'created_at', 'updated_at']) && in_array($direction, ['asc', 'desc'])) {
    ...
    {
        return [
            ['key' => 'name',       'label' => __('tap_changer_types.name'),     'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'code',       'label' => __('tap_changer_types.code'),     'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'is_active',  'label' => __('tap_changer_types.is_active'), 'type' => 'boolean', 'operators' => ['=']],
