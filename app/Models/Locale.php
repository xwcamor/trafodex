...
class Locale extends Model
{
    ...
    public function scopeFilter(Builder $query, Request|array $filters): Builder
    {
        ...
        if ($sort === 'language' && in_array($direction, ['asc', 'desc'])) {
            // Orden por nombre del idioma (la columna muestra el nombre, no el id)
            // vía left join. El select explícito de locales.* evita colisión.
            $query->leftJoin('languages', 'languages.id', '=', "{$tbl}.language_id")
                  ->orderBy('languages.name', $direction);
        } elseif (in_array($sort, ['id', 'name', 'code', 'language_id', 'is_active', 'created_at', 'updated_at', 'deleted_at']) && in_array($direction, ['asc', 'desc'])) {
