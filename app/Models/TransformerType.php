...
/**
 * TransformerType — catálogo de tipos de transformador (Potencia, Distribución, Horno…).
 *
 * Es a la vez:
 *   - Eje de diagnóstico del motor (las reglas cambian según el tipo) → ruleSets().
 *   - Módulo de gestión CRUD (catálogo global, sin tenant). Mantiene
 *     SoftDeletes + Auditable + HasFavorites. Campos: name, code, sort_order,
 *     is_active.
 */
class TransformerType extends Model
{
    ...
    protected string $auditModule = 'transformer_types';

    protected $fillable = [
        'slug', 'name', 'code', 'shape', 'is_active', 'sort_order',
    ...
    /** Reglas de diagnóstico que dependen de este tipo de transformador (motor). */
    public function ruleSets(): HasMany
    {
        return $this->hasMany(RuleSet::class);
    }

    /** Transformadores de este tipo (FK transformer_type_id). */
    ...
    /**
     * scopeFilter — mismo patrón que Customer, sobre la tabla transformer_types.
    ...
    public function scopeFilter($query, $request)
    {
        ...
        if (in_array($sort, ['id', 'name', 'code', 'shape', 'is_active', 'sort_order', 'created_at', 'updated_at']) && in_array($direction, ['asc', 'desc'])) {
    ...
    {
        return [
            ['key' => 'name',       'label' => __('transformer_types.name'),     'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'code',       'label' => __('transformer_types.code'),     'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'is_active',  'label' => __('transformer_types.is_active'), 'type' => 'boolean', 'operators' => ['=']],
