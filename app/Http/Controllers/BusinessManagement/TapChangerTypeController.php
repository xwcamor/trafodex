...
use App\Http\Requests\BusinessManagement\TapChangerType\BulkDeleteTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\BulkRestoreTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\BulkSetActiveTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\DeleteTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\EditAllUpdateTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\ForceDeleteTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\ImportTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\StoreTapChangerTypeRequest;
use App\Http\Requests\BusinessManagement\TapChangerType\UpdateTapChangerTypeRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\TapChangerTypes\GenerateTapChangerTypesCsvJob;
use App\Jobs\BusinessManagement\TapChangerTypes\GenerateTapChangerTypesExcelJob;
use App\Jobs\BusinessManagement\TapChangerTypes\GenerateTapChangerTypesPdfJob;
use App\Jobs\BusinessManagement\TapChangerTypes\GenerateTapChangerTypesWordJob;
use App\Models\AuditLog;
use App\Models\TapChangerType;
use App\Services\BusinessManagement\TapChangerTypeService;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;

class TapChangerTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // tap_changer_types es catalogo global (sin tenant) — solo eager-load creator.
        $with = ['creator:id,name,email'];

        $tapChangerTypes = TapChangerType::query()
            ->select('tap_changer_types.*')
            ->with($with)
        ...
        $totalUnfiltered = TapChangerType::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('TapChangerTypes/Index', [
            'tapChangerTypes' => array_merge($tapChangerTypes->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('tap_changer_types'),
            'filters' => [
            ...
            'filterSchema'   => TapChangerType::filterSchema(),
        ]);
    }
    ...
    public function show(Request $request, TapChangerType $tapChangerType)
    {
        $tapChangerType->load(['creator:id,name,email', 'deleter:id,name,email']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', TapChangerType::class)
                    ->where('auditable_id', $tapChangerType->id)
        ...
        return inertia('TapChangerTypes/Show', [
            'tapChangerType' => $this->payload($tapChangerType, withAudit: true),
            'recordAudit'  => $this->recordAuditMeta($tapChangerType),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('TapChangerTypes/Form', [
            'tapChangerType'        => null,
        ]);
    }

    public function store(StoreTapChangerTypeRequest $request, TapChangerTypeService $service): RedirectResponse
    {
        ...
        if ($tenant) {
            ...
            if ($max > 0 && TapChangerType::count() >= $max) {
        ...
        return redirect()
            ->route('business_management.tap_changer_types.index')
            ->with('success', __('tap_changer_types.created'));
    }

    public function edit(TapChangerType $tapChangerType)
    {
        return inertia('TapChangerTypes/Form', [
            'tapChangerType'        => $this->payload($tapChangerType),
        ]);
    }


    public function update(UpdateTapChangerTypeRequest $request, TapChangerType $tapChangerType, TapChangerTypeService $service): RedirectResponse
    {
        $service->update($tapChangerType, $request->validated());

        return redirect()
            ->route('business_management.tap_changer_types.index')
            ->with('success', __('tap_changer_types.saved'));
    }

    public function delete(TapChangerType $tapChangerType)
    {
        return inertia('TapChangerTypes/Delete', [
            'tapChangerType' => $this->payload($tapChangerType),
        ]);
    }

    public function deleteSave(DeleteTapChangerTypeRequest $request, TapChangerType $tapChangerType, TapChangerTypeService $service): RedirectResponse
    {
        $service->delete($tapChangerType, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$tapChangerType->id]);

        return redirect()
            ->route('business_management.tap_changer_types.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$tapChangerType->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['tap_changer_types.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('TapChangerTypes/Trash', [
    ...
    public function restore(Request $request, $slug, TapChangerTypeService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = TapChangerType::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.tap_changer_types.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $tapChangerTypes = TapChangerType::query()
            ->filter($request)
            ->select('tap_changer_types.id', 'tap_changer_types.slug', 'tap_changer_types.name', 'tap_changer_types.code', 'tap_changer_types.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('TapChangerTypes/EditAll', [
            'tapChangerTypes' => $tapChangerTypes,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateTapChangerTypeRequest $request, TapChangerTypeService $service): RedirectResponse
    {
        $touched = $service->editAllUpdate($request->validated()['changes']);

        return redirect()
            ->route('business_management.tap_changer_types.edit_all')
     ...
     */
    public function duplicate(Request $request, TapChangerType $tapChangerType, TapChangerTypeService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($tapChangerType);
    ...
    public function bulkRestore(BulkRestoreTapChangerTypeRequest $request, TapChangerTypeService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteTapChangerTypeRequest $request, $slug, TapChangerTypeService $service): RedirectResponse
    {
        $model = TapChangerType::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(TapChangerType $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateTapChangerTypesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateTapChangerTypesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateTapChangerTypesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateTapChangerTypesWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('tap_changer_types', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('tap_changer_types.export_limit_exceeded', [
    ...
    protected function countForExport(array $options): int
    {
        ...
        if ($scope === 'all') {
            return TapChangerType::query()->count();
        }
        // Filters como Request para reusar scopeFilter.
        $fakeReq = new Request($options['filters'] ?? []);
        return TapChangerType::query()->filter($fakeReq)->count();
    }
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\TapChangerTypes\TapChangerTypesImportTemplate(),
            __('tap_changer_types.import_template_filename')
        );
    }

    public function import(ImportTapChangerTypeRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a tapChangerTypes de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('tap_changer_types.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\TapChangerTypes\TapChangerTypesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('TapChangerTypesImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteTapChangerTypeRequest $request, TapChangerTypeService $service): RedirectResponse
    {
        $data   = $request->validated();
     ...
     */
    public function undoLastDelete(Request $request, TapChangerTypeService $service): RedirectResponse
    {
        $claim = session('tap_changer_types.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('tap_changer_types.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveTapChangerTypeRequest $request, TapChangerTypeService $service): RedirectResponse
    {
        $data   = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('tap_changer_types.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.tap_changer_types.index'),
