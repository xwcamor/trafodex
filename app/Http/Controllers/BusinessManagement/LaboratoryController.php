...
use App\Http\Requests\BusinessManagement\Laboratory\BulkDeleteLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\BulkRestoreLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\BulkSetActiveLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\DeleteLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\EditAllUpdateLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\ForceDeleteLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\ImportLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\StoreLaboratoryRequest;
use App\Http\Requests\BusinessManagement\Laboratory\UpdateLaboratoryRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\Laboratories\GenerateLaboratoriesCsvJob;
use App\Jobs\BusinessManagement\Laboratories\GenerateLaboratoriesExcelJob;
use App\Jobs\BusinessManagement\Laboratories\GenerateLaboratoriesPdfJob;
use App\Jobs\BusinessManagement\Laboratories\GenerateLaboratoriesWordJob;
use App\Models\AuditLog;
use App\Models\Laboratory;
use App\Services\BusinessManagement\LaboratoryService;
...
class LaboratoryController extends Controller
{
    ...
    /** Pone el candado al laboratorio (super → nivel sistema; admin → nivel tenant). */
    public function lock(Request $request, Laboratory $laboratory): RedirectResponse
    {
        return $this->applyLock($laboratory, $request);
    }

    /** Saca el candado (un admin no puede quitar un candado del super). */
    public function unlock(Request $request, Laboratory $laboratory): RedirectResponse
    {
        return $this->applyUnlock($laboratory, $request);
    }
    ...
    public function index(Request $request)
    {
        ...
        // laboratories es per-tenant + globales; el super ve cross-tenant, carga el tenant.
        ...
        $totalUnfiltered = Laboratory::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('Laboratories/Index', [
            'laboratories' => array_merge($laboratories->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('laboratories'),
            'filters' => [
            ...
            'filterSchema'   => Laboratory::filterSchema(),
        ]);
    }
    ...
    {
        $laboratory->load(['creator:id,name,email', 'deleter:id,name,email', 'locker:id,name']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', Laboratory::class)
                    ->where('auditable_id', $laboratory->id)
        ...
        return inertia('Laboratories/Show', [
            'laboratory' => array_merge(
                $this->payload($laboratory, withAudit: true),
                ['lock' => $this->lockMeta($laboratory, $request)],
            ),
            'recordAudit'  => $this->recordAuditMeta($laboratory),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('Laboratories/Form', [
            'laboratory'        => null,
        ]);
    }

    public function quickStore(StoreLaboratoryRequest $request, LaboratoryService $service): \Illuminate\Http\JsonResponse
    {
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
            if ($max > 0 && Laboratory::count() >= $max) {
    ...
    public function store(StoreLaboratoryRequest $request, LaboratoryService $service): RedirectResponse
    {
        ...
        return redirect()
            ->route('business_management.laboratories.index')
            ->with('success', __('laboratories.created'));
    }

    public function edit(Laboratory $laboratory)
    {
        // Registro bloqueado (Lockable): ni se abre el formulario de edición.
        abort_if($laboratory->is_locked, 403, __('locks.cannot_edit_locked'));

        return inertia('Laboratories/Form', [
            'laboratory'        => $this->payload($laboratory),
        ]);
    }


    public function update(UpdateLaboratoryRequest $request, Laboratory $laboratory, LaboratoryService $service): RedirectResponse
    {
        $service->update($laboratory, $request->validated());

        return redirect()
            ->route('business_management.laboratories.index')
            ->with('success', __('laboratories.saved'));
    }

    public function delete(Laboratory $laboratory)
    {
        // Registro bloqueado (Lockable): ni se abre la confirmación de borrado.
        abort_if($laboratory->is_locked, 403, __('locks.cannot_delete_locked'));

        return inertia('Laboratories/Delete', [
            'laboratory' => $this->payload($laboratory),
        ]);
    }

    public function deleteSave(DeleteLaboratoryRequest $request, Laboratory $laboratory, LaboratoryService $service): RedirectResponse
    {
        $service->delete($laboratory, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$laboratory->id]);

        return redirect()
            ->route('business_management.laboratories.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$laboratory->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['laboratories.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('Laboratories/Trash', [
    ...
    public function restore(Request $request, $slug, LaboratoryService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = Laboratory::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.laboratories.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $laboratories = Laboratory::query()
            ->filter($request)
            ->select('laboratories.id', 'laboratories.slug', 'laboratories.name', 'laboratories.code', 'laboratories.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('Laboratories/EditAll', [
            'laboratories' => $laboratories,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateLaboratoryRequest $request, LaboratoryService $service): RedirectResponse
    {
        $changes = $request->validated()['changes'];

        // Excluir registros BLOQUEADOS (Lockable) de la edición masiva.
        $ids = array_column($changes, 'id');
        [, $lockedIds] = $this->splitLockedIds(Laboratory::class, $ids);
        if (!empty($lockedIds)) {
        ...
        return redirect()
            ->route('business_management.laboratories.edit_all')
            ->with('success', $msg);
    }
     ...
     */
    public function duplicate(Request $request, Laboratory $laboratory, LaboratoryService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($laboratory);
    ...
    public function bulkRestore(BulkRestoreLaboratoryRequest $request, LaboratoryService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteLaboratoryRequest $request, $slug, LaboratoryService $service): RedirectResponse
    {
        $model = Laboratory::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(Laboratory $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateLaboratoriesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateLaboratoriesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateLaboratoriesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateLaboratoriesWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('laboratories', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('laboratories.export_limit_exceeded', [
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\Laboratories\LaboratoriesImportTemplate(),
            __('laboratories.import_template_filename')
        );
    }

    public function import(ImportLaboratoryRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a laboratories de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('laboratories.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\Laboratories\LaboratoriesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('LaboratoriesImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteLaboratoryRequest $request, LaboratoryService $service): RedirectResponse
    {
        $data = $request->validated();

        // Excluir registros BLOQUEADOS (Lockable): no se borran en masa.
        [$allowedIds, $lockedIds] = $this->splitLockedIds(Laboratory::class, $data['ids']);
        if (empty($allowedIds)) {
     ...
     */
    public function undoLastDelete(Request $request, LaboratoryService $service): RedirectResponse
    {
        $claim = session('laboratories.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('laboratories.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveLaboratoryRequest $request, LaboratoryService $service): RedirectResponse
    {
        $data = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('laboratories.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.laboratories.index'),
