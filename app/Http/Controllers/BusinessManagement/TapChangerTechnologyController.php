...
use App\Http\Requests\BusinessManagement\TapChangerTechnology\BulkDeleteTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\BulkRestoreTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\BulkSetActiveTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\DeleteTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\EditAllUpdateTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\ForceDeleteTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\ImportTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\StoreTapChangerTechnologyRequest;
use App\Http\Requests\BusinessManagement\TapChangerTechnology\UpdateTapChangerTechnologyRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\TapChangerTechnologies\GenerateTapChangerTechnologiesCsvJob;
use App\Jobs\BusinessManagement\TapChangerTechnologies\GenerateTapChangerTechnologiesExcelJob;
use App\Jobs\BusinessManagement\TapChangerTechnologies\GenerateTapChangerTechnologiesPdfJob;
use App\Jobs\BusinessManagement\TapChangerTechnologies\GenerateTapChangerTechnologiesWordJob;
...
class TapChangerTechnologyController extends Controller
{
    use \App\Traits\BuildsRecordAudit;
    use \App\Http\Controllers\Concerns\HandlesRecordLocking;

    /** Pone el candado al registro (super → nivel sistema; admin → nivel tenant). */
    public function lock(Request $request, TapChangerTechnology $tapChangerTechnology): RedirectResponse
    {
        return $this->applyLock($tapChangerTechnology, $request);
    }

    /** Saca el candado (un admin no puede quitar un candado del super). */
    public function unlock(Request $request, TapChangerTechnology $tapChangerTechnology): RedirectResponse
    {
        return $this->applyUnlock($tapChangerTechnology, $request);
    }
    ...
    public function index(Request $request)
    {
        ...
        // tap_changer_technologies es per-tenant + globales; el super ve cross-tenant, carga el tenant.
        ...
        $tapChangerTechnologies = TapChangerTechnology::query()
            ->select('tap_changer_technologies.*')
            ->with($with)
        ...
        $totalUnfiltered = TapChangerTechnology::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('TapChangerTechnologies/Index', [
            'tapChangerTechnologies' => array_merge($tapChangerTechnologies->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('tap_changer_technologies'),
            'filters' => [
            ...
            'filterSchema'   => TapChangerTechnology::filterSchema(),
        ]);
    }
    ...
    public function show(Request $request, TapChangerTechnology $tapChangerTechnology)
    {
        $tapChangerTechnology->load(['creator:id,name,email', 'deleter:id,name,email', 'locker:id,name']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', TapChangerTechnology::class)
                    ->where('auditable_id', $tapChangerTechnology->id)
        ...
        return inertia('TapChangerTechnologies/Show', [
            'tapChangerTechnology' => array_merge(
                $this->payload($tapChangerTechnology, withAudit: true),
                ['lock' => $this->lockMeta($tapChangerTechnology, $request)],
            ),
            'recordAudit'  => $this->recordAuditMeta($tapChangerTechnology),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('TapChangerTechnologies/Form', [
            'tapChangerTechnology'        => null,
        ]);
    }

    public function quickStore(StoreTapChangerTechnologyRequest $request, TapChangerTechnologyService $service): \Illuminate\Http\JsonResponse
    {
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
            if ($max > 0 && TapChangerTechnology::count() >= $max) {
    ...
    public function store(StoreTapChangerTechnologyRequest $request, TapChangerTechnologyService $service): RedirectResponse
    {
        ...
        return redirect()
            ...
            ->with('success', __('tap_changer_technologies.created'));
    }

    public function edit(TapChangerTechnology $tapChangerTechnology)
    {
        // Registro bloqueado (Lockable): ni se abre el formulario de edición.
        abort_if($tapChangerTechnology->is_locked, 403, __('locks.cannot_edit_locked'));

        return inertia('TapChangerTechnologies/Form', [
            'tapChangerTechnology'        => $this->payload($tapChangerTechnology),
        ]);
    }


    public function update(UpdateTapChangerTechnologyRequest $request, TapChangerTechnology $tapChangerTechnology, TapChangerTechnologyService $service): RedirectResponse
    {
        $service->update($tapChangerTechnology, $request->validated());

        return redirect()
            ->route('business_management.tap_changer_technologies.index')
            ->with('success', __('tap_changer_technologies.saved'));
    }

    public function delete(TapChangerTechnology $tapChangerTechnology)
    {
        // Registro bloqueado (Lockable): ni se abre la confirmación de borrado.
        abort_if($tapChangerTechnology->is_locked, 403, __('locks.cannot_delete_locked'));

        return inertia('TapChangerTechnologies/Delete', [
            'tapChangerTechnology' => $this->payload($tapChangerTechnology),
        ]);
    }

    public function deleteSave(DeleteTapChangerTechnologyRequest $request, TapChangerTechnology $tapChangerTechnology, TapChangerTechnologyService $service): RedirectResponse
    {
        $service->delete($tapChangerTechnology, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$tapChangerTechnology->id]);

        return redirect()
            ->route('business_management.tap_changer_technologies.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$tapChangerTechnology->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['tap_changer_technologies.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('TapChangerTechnologies/Trash', [
    ...
    public function restore(Request $request, $slug, TapChangerTechnologyService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = TapChangerTechnology::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.tap_changer_technologies.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $tapChangerTechnologies = TapChangerTechnology::query()
            ->filter($request)
            ->select('tap_changer_technologies.id', 'tap_changer_technologies.slug', 'tap_changer_technologies.name', 'tap_changer_technologies.code', 'tap_changer_technologies.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('TapChangerTechnologies/EditAll', [
            'tapChangerTechnologies' => $tapChangerTechnologies,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateTapChangerTechnologyRequest $request, TapChangerTechnologyService $service): RedirectResponse
    {
        $changes = $request->validated()['changes'];

        // Excluir registros BLOQUEADOS (Lockable) de la edición masiva.
        $ids = array_column($changes, 'id');
        [, $lockedIds] = $this->splitLockedIds(TapChangerTechnology::class, $ids);
        if (!empty($lockedIds)) {
     ...
     */
    public function duplicate(Request $request, TapChangerTechnology $tapChangerTechnology, TapChangerTechnologyService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($tapChangerTechnology);
    ...
    public function bulkRestore(BulkRestoreTapChangerTechnologyRequest $request, TapChangerTechnologyService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteTapChangerTechnologyRequest $request, $slug, TapChangerTechnologyService $service): RedirectResponse
    {
        $model = TapChangerTechnology::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(TapChangerTechnology $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateTapChangerTechnologiesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateTapChangerTechnologiesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateTapChangerTechnologiesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateTapChangerTechnologiesWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('tap_changer_technologies', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('tap_changer_technologies.export_limit_exceeded', [
    ...
    protected function countForExport(array $options): int
    {
        ...
        if ($scope === 'all') {
            return TapChangerTechnology::query()->count();
        }
        // Filters como Request para reusar scopeFilter.
        $fakeReq = new Request($options['filters'] ?? []);
        return TapChangerTechnology::query()->filter($fakeReq)->count();
    }
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\TapChangerTechnologies\TapChangerTechnologiesImportTemplate(),
            __('tap_changer_technologies.import_template_filename')
        );
    }

    public function import(ImportTapChangerTechnologyRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a tapChangerTechnologies de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('tap_changer_technologies.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\TapChangerTechnologies\TapChangerTechnologiesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('TapChangerTechnologiesImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteTapChangerTechnologyRequest $request, TapChangerTechnologyService $service): RedirectResponse
    {
        $data = $request->validated();

        // Excluir registros BLOQUEADOS (Lockable): no se borran en masa.
        [$allowedIds, $lockedIds] = $this->splitLockedIds(TapChangerTechnology::class, $data['ids']);
        if (empty($allowedIds)) {
     ...
     */
    public function undoLastDelete(Request $request, TapChangerTechnologyService $service): RedirectResponse
    {
        $claim = session('tap_changer_technologies.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('tap_changer_technologies.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveTapChangerTechnologyRequest $request, TapChangerTechnologyService $service): RedirectResponse
    {
        $data = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('tap_changer_technologies.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.tap_changer_technologies.index'),
