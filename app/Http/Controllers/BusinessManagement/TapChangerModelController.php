...
use App\Http\Requests\BusinessManagement\TapChangerModel\BulkDeleteTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\BulkRestoreTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\BulkSetActiveTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\DeleteTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\EditAllUpdateTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\ForceDeleteTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\ImportTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\StoreTapChangerModelRequest;
use App\Http\Requests\BusinessManagement\TapChangerModel\UpdateTapChangerModelRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\TapChangerModels\GenerateTapChangerModelsCsvJob;
use App\Jobs\BusinessManagement\TapChangerModels\GenerateTapChangerModelsExcelJob;
use App\Jobs\BusinessManagement\TapChangerModels\GenerateTapChangerModelsPdfJob;
use App\Jobs\BusinessManagement\TapChangerModels\GenerateTapChangerModelsWordJob;
...
class TapChangerModelController extends Controller
{
    ...
    public function lock(Request $request, TapChangerModel $tapChangerModel): RedirectResponse
    {
        return $this->applyLock($tapChangerModel, $request);
    }

    /** Saca el candado (un admin no puede quitar un candado del super). */
    public function unlock(Request $request, TapChangerModel $tapChangerModel): RedirectResponse
    {
        return $this->applyUnlock($tapChangerModel, $request);
    }
    ...
    public function index(Request $request)
    {
        ...
        // tap_changer_models es per-tenant + globales; el super ve cross-tenant, carga el tenant.
        ...
        $tapChangerModels = TapChangerModel::query()
            ->select('tap_changer_models.*')
            ->with($with)
        ...
        $totalUnfiltered = TapChangerModel::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('TapChangerModels/Index', [
            'tapChangerModels' => array_merge($tapChangerModels->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('tap_changer_models'),
            'filters' => [
            ...
            'filterSchema'   => TapChangerModel::filterSchema(),
        ]);
    }
    ...
    public function show(Request $request, TapChangerModel $tapChangerModel)
    {
        $tapChangerModel->load(['creator:id,name,email', 'deleter:id,name,email', 'locker:id,name']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', TapChangerModel::class)
                    ->where('auditable_id', $tapChangerModel->id)
        ...
        return inertia('TapChangerModels/Show', [
            'tapChangerModel' => array_merge(
                $this->payload($tapChangerModel, withAudit: true),
                ['lock' => $this->lockMeta($tapChangerModel, $request)],
            ),
            'recordAudit'  => $this->recordAuditMeta($tapChangerModel),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('TapChangerModels/Form', [
            'tapChangerModel'        => null,
        ]);
    }

    public function quickStore(StoreTapChangerModelRequest $request, TapChangerModelService $service): \Illuminate\Http\JsonResponse
    {
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
            if ($max > 0 && TapChangerModel::count() >= $max) {
    ...
    public function store(StoreTapChangerModelRequest $request, TapChangerModelService $service): RedirectResponse
    {
        ...
        return redirect()
            ->route('business_management.tap_changer_models.index')
            ->with('success', __('tap_changer_models.created'));
    }

    public function edit(TapChangerModel $tapChangerModel)
    {
        // Registro bloqueado (Lockable): ni se abre el formulario de edición.
        abort_if($tapChangerModel->is_locked, 403, __('locks.cannot_edit_locked'));

        return inertia('TapChangerModels/Form', [
            'tapChangerModel'        => $this->payload($tapChangerModel),
        ]);
    }


    public function update(UpdateTapChangerModelRequest $request, TapChangerModel $tapChangerModel, TapChangerModelService $service): RedirectResponse
    {
        $service->update($tapChangerModel, $request->validated());

        return redirect()
            ->route('business_management.tap_changer_models.index')
            ->with('success', __('tap_changer_models.saved'));
    }

    public function delete(TapChangerModel $tapChangerModel)
    {
        // Registro bloqueado (Lockable): ni se abre la confirmación de borrado.
        abort_if($tapChangerModel->is_locked, 403, __('locks.cannot_delete_locked'));

        return inertia('TapChangerModels/Delete', [
            'tapChangerModel' => $this->payload($tapChangerModel),
        ]);
    }

    public function deleteSave(DeleteTapChangerModelRequest $request, TapChangerModel $tapChangerModel, TapChangerModelService $service): RedirectResponse
    {
        $service->delete($tapChangerModel, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$tapChangerModel->id]);

        return redirect()
            ->route('business_management.tap_changer_models.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$tapChangerModel->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['tap_changer_models.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        $tapChangerModels = TapChangerModel::onlyTrashed()
        ...
        return inertia('TapChangerModels/Trash', [
    ...
    public function restore(Request $request, $slug, TapChangerModelService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = TapChangerModel::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.tap_changer_models.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $tapChangerModels = TapChangerModel::query()
            ->filter($request)
            ->select('tap_changer_models.id', 'tap_changer_models.slug', 'tap_changer_models.name', 'tap_changer_models.code', 'tap_changer_models.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('TapChangerModels/EditAll', [
            'tapChangerModels' => $tapChangerModels,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateTapChangerModelRequest $request, TapChangerModelService $service): RedirectResponse
    {
        $changes = $request->validated()['changes'];

        // Excluir registros BLOQUEADOS (Lockable) de la edición masiva.
        $ids = array_column($changes, 'id');
        [, $lockedIds] = $this->splitLockedIds(TapChangerModel::class, $ids);
        if (!empty($lockedIds)) {
    ...
    /**
     * Clona el tapChangerModel. Sufijo "(copia)" con sanity guard de 100 intentos.
     */
    public function duplicate(Request $request, TapChangerModel $tapChangerModel, TapChangerModelService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($tapChangerModel);
    ...
    public function bulkRestore(BulkRestoreTapChangerModelRequest $request, TapChangerModelService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteTapChangerModelRequest $request, $slug, TapChangerModelService $service): RedirectResponse
    {
        $model = TapChangerModel::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(TapChangerModel $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateTapChangerModelsCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateTapChangerModelsExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateTapChangerModelsPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateTapChangerModelsWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('tap_changer_models', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('tap_changer_models.export_limit_exceeded', [
    ...
    protected function countForExport(array $options): int
    {
        ...
        if ($scope === 'all') {
            return TapChangerModel::query()->count();
        }
        // Filters como Request para reusar scopeFilter.
        $fakeReq = new Request($options['filters'] ?? []);
        return TapChangerModel::query()->filter($fakeReq)->count();
    }
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\TapChangerModels\TapChangerModelsImportTemplate(),
            __('tap_changer_models.import_template_filename')
        );
    }

    public function import(ImportTapChangerModelRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a tapChangerModels de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('tap_changer_models.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\TapChangerModels\TapChangerModelsImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('TapChangerModelsImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteTapChangerModelRequest $request, TapChangerModelService $service): RedirectResponse
    {
        $data = $request->validated();

        // Excluir registros BLOQUEADOS (Lockable): no se borran en masa.
        [$allowedIds, $lockedIds] = $this->splitLockedIds(TapChangerModel::class, $data['ids']);
        if (empty($allowedIds)) {
     ...
     */
    public function undoLastDelete(Request $request, TapChangerModelService $service): RedirectResponse
    {
        $claim = session('tap_changer_models.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('tap_changer_models.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveTapChangerModelRequest $request, TapChangerModelService $service): RedirectResponse
    {
        $data = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('tap_changer_models.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            'user_id'        => auth()->id(),
            'event'          => 'export_queued',
            'auditable_type' => TapChangerModel::class,
            'auditable_id'   => null,
            ...
            'url'        => route('business_management.tap_changer_models.index'),
